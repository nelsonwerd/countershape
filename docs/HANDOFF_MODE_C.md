...
- **Pipeline phase:** U0–U6, P07A/U6b, every P07B-A/B boundary, and P07B-C through C3L are sealed and strict-clean. C3F B future-surface admission maintenance is the active exact ten-path `SOURCE_FULL` unit and remains `UNRECEIPTED`; sealed C3L commit `efa918928246c7793f4ad7201c003020e3a42193`, tree `d9187c6449392c269a385b53c563ba4411112460`, is the closed legacy-lattice prerequisite. C3 stays blocked until C3F independently closes.
- **Git:** repository is on `codex/countershape-autopilot` at sealed C3L HEAD `efa918928246c7793f4ad7201c003020e3a42193`, tree `d9187c6449392c269a385b53c563ba4411112460`. The dirty tree is limited to C3F's exact ten-path roster at `sha256:f64293895a1b824faafddf236757d381c813b59c40286225bb1c352d5c812c1a`; sealed C3L retains its exact nine-path roster at `sha256:b3624e1337681e0909e6c074246103e066118a5924e8f790e7d1f42359da9dc0`, and C3 remains frozen at `sha256:c0050817c739e6bf7505105113ebbfd2c2504674f93f14e34daf0609f7fad0fb`. C3 stash object `f7160bbbc09245a1026eb6396ac76282ab913192` and earlier checkpoints `fa737661e61c36c10ce793f64852836edec20065` and `3e766715609680adaef4678fc9aeaa9e3a8bae09` remain retained without dropping; older receipt-history stashes remain untouched.
- **didrun:** installed globally and unchanged. C3L's sealed final ledger is `.didrun-history/2026-07-19-p07b-c-c3l-final/.didrun/`; its exact-commit HTML is `.countershape/evidence/p07b-c-c3l-final-efa918928246.html`. C3F uses a zero-claim development ledger and a separate fresh ten-claim final ledger; no C3L event is reused. Before final-ledger rotation, any live `.didrun/` is C3F development evidence only; after explicit rotation, the fresh ten-claim ledger is C3F final evidence pending commit, seal, and strict closure.
- **Current baseline:** from a fresh repository-root shell, run `/opt/homebrew/bin/node tools/verify-current.mjs`. C3F conditionally admits only the complete exact four-pair future C3 target-codec surface through B, preserves the historical zero-member state, and then runs B, its hostile self-test, and the full cumulative verifier through didrun. A direct pass has no didrun grade. The sealed C3P receipt block remains immutable source evidence.
...
The active C3F working unit remains `UNRECEIPTED` and cannot grade its own commit, tree, Git note, or strict result.
...
### Active C3F B future-surface admission maintenance

C3M's earlier “synthetic C3PB receipt phase” heading is retained in its status as historical checker evidence. The exact phrase “C3M receipt-phase checker maintenance is the active `SOURCE_FULL` unit” is retained here only as inert compatibility history, as is preserved C3PB-era stash object `5eeb848c334ad3a8a44e4cf61fa298e452188ab1`. In the live handoff, C3M, C3PB, C3A, and C3L are closed and C3F is the sole active unit.

The phase classifier still recognizes complete C3M-active, C3PB-active, C3A-active, and C3L-active tuples so stale state receives a precise diagnostic, but none is currently admissible. Both receipt-absent and receipt-present synthetic plans must preserve exactly one complete C3F tuple at its canonical Markdown locations: exactly one exact line for each of four named metadata keys inside `## Current state`, then the matching active subsection heading and receipt-presence paragraph. Missing, duplicated, relocated-to-history, same-line historical embedding, HTML-commented, fenced, raw-HTML-wrapped, mixed, or coherent whole-tuple downgrade state fails. Canonical C3F receipt markers and grade forms are forbidden across every C3F-owned Markdown file.
...
The C3P receipt declaration and one exact source-receipt block remain closed source evidence. C3L is independently sealed; C3F changes neither receipt payload nor any C3P/C3PB/C3A/C3L grade.
...
C3F exists because the first resumed C3 architecture baseline to reach inherited B, after an in-scope source-shape correction, truthfully failed on three exact codec identifiers that C3 requires in addition to the already admitted target type. C3F's production-architecture delta changes only B's admission policy: historical zero-member state remains valid, while the future target surface is admitted only as the complete exact four-pair bundle. Every partial, relocated, renamed, Unicode-suffixed, or extra family surface remains foreign. The three-event, zero-claim trigger ledger is preserved at `.didrun-history/2026-07-19-p07b-c-c3-build-loop-post-c3l-pre-c3f/.didrun/`. C3's forty-path source roster is unchanged in stash `f7160bbbc09245a1026eb6396ac76282ab913192`; no C3 runtime, target, or behavior claim is implied.
...
The phase checker likewise retains the following pre-seal compatibility statement. It is historical evidence only; C1E, C1B, C2, C2M, C2B, C3P, C3V, C3M, C3PB, C3A, and C3L subsequently sealed, and C3F is now the live unit:
...
1. Run `git status --short --branch`; confirm branch `codex/countershape-autopilot`, sealed C3L parent `efa918928246c7793f4ad7201c003020e3a42193`, tree `d9187c6449392c269a385b53c563ba4411112460`, and changes limited to C3F's exact ten-path roster at `sha256:f64293895a1b824faafddf236757d381c813b59c40286225bb1c352d5c812c1a`. Retain stash object `f7160bbbc09245a1026eb6396ac76282ab913192`, earlier checkpoints `fa737661e61c36c10ce793f64852836edec20065` and `3e766715609680adaef4678fc9aeaa9e3a8bae09`, and every earlier stash/failed/superseded ledger by full identity. No `.didrun` or `.didrun-history` path may be staged or tracked.
2. Finish the C3F B all-or-none surface, plan-phase, unit-scope, and cumulative-verifier hostile loops. Require historical zero-member admission, exact complete four-pair admission, and fail-closed partial/relocated/renamed/Unicode/extra cases. Any red result remains unclaimed and must be fixed rather than bypassed.
3. Archive the zero-claim development ledger; rotate any development-populated `.countershape/p07bc-c3f-final` root; recreate that root plus `home`, `tmp`, `gotmp`, `gocache`, `gopath`, and `gomodcache` as owned mode-`0700` nonsymlink directories. On one exact staged tree, run and immediately claim only C3F's ten declared commands using the manifest's common `/usr/bin/env -i` prefix, commit as `fix: admit exact C3 target codec surface through B`, seal, inspect the Git note, loop `NO_COLOR=1 didrun verify --strict` to exit `0`, generate the exact-commit HTML report, and archive the final ledger. The chain gate binds exact argv, repository cwd, stable didrun wrapper fingerprint, exact claim shapes, common staged tree, and absent seal for the first nine events, then validates its own live Node/invocation/effective bindings and private directories. It also binds current `HEAD` to sealed C3L's exact commit/tree/parent/subject and exact `refs/notes/didrun` blob/body/12-claim coverage before the C3F commit. Event ten cannot self-prove its outer launcher because didrun appends that event after exit; inspect the recorded event honestly, without treating it as part of the preceding-chain claim. The fingerprint is not a full environment attestation.
4. Only after C3F closes, revalidate and apply stash `f7160bbbc09245a1026eb6396ac76282ab913192` without dropping. Any overlap must preserve the sealed C3A/C3L/C3F repairs, and the post-apply inventory must remain within C3's frozen forty-path roster. Bind sealed C3F as C3's direct predecessor while retaining sealed C3L as C3F's parent and sealed C3A as C3L's parent, then resume the full C3 build-loop.
...
5. the current bounded shippable unit: exact ten-path C3F `SOURCE_FULL`; sealed C3L commit `efa918928246c7793f4ad7201c003020e3a42193` is the closed legacy-lattice prerequisite, the C3P source grades below remain bound only to the independently sealed C3P source, C3F remains `UNRECEIPTED`, C3 is blocked on its closure, and every unrun live/runtime capability remains excluded and `UNRECEIPTED`.
