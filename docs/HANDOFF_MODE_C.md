...
- **Pipeline phase:** U0–U6, P07A/U6b, every P07B-A/B boundary, and P07B-C through C3PB are sealed and strict-clean. C3A cumulative-admission maintenance is the active exact thirteen-path `SOURCE_FULL` unit and remains `UNRECEIPTED`; sealed C3PB commit `13369122ba7d5557eba1949095c1135a41843070`, tree `d710d9b249786f3fb648f566ae542f1d9ddec180`, is the closed receipt prerequisite. C3 stays blocked until C3A independently closes. Historical compatibility anchor: the sentence “C3M receipt-phase checker maintenance is the active `SOURCE_FULL` unit” is retained as inert phase history.
- **Git:** repository is on `codex/countershape-autopilot` at sealed C3PB HEAD `13369122ba7d5557eba1949095c1135a41843070`, tree `d710d9b249786f3fb648f566ae542f1d9ddec180`. The dirty tree is limited to C3A's exact thirteen-path roster at `sha256:cc9bb98ceeb2f281336d3e235377d3cdb8920664f833baea830a84b847621188`; sealed C3PB retains its exact three-path roster at `sha256:531cbe600cf2747752749890ea9a15c3d0126e498da6838d4f6faeb1a0d7e885`, and the closed interposed C3M roster remains `sha256:58e1f6fba95cf3b4312136eea5bb5c64af89a6958711aca84181a1874aa03240`. Stash objects `3e766715609680adaef4678fc9aeaa9e3a8bae09`, `5eeb848c334ad3a8a44e4cf61fa298e452188ab1`, `229a60c0630e296b67faa3c81b91fac73f9f63ff`, and `1d2c14f160ff42d509d96219512fa413830a9b1d` remain retained; address each by full identity.
- **didrun:** installed globally and unchanged. C3PB's sealed final ledger is `.didrun-history/2026-07-19-p07b-c-c3pb-final/.didrun/`; its exact-commit HTML is `.countershape/evidence/p07b-c-c3pb-final-13369122ba7d.html`. C3A uses a zero-claim development ledger and a separate fresh eleven-claim final ledger; no C3PB event is reused. Any live `.didrun/` created now is C3A development evidence only.
- **Current baseline:** from a fresh repository-root shell, run `/opt/homebrew/bin/node tools/verify-current.mjs`. C3A repairs the inherited B architecture admission and the canonical runtime-version bound, then runs its direct model/architecture gates plus the full cumulative verifier through didrun. A direct pass has no didrun grade. The sealed C3P receipt block remains immutable source evidence.
...
The active C3A working unit remains `UNRECEIPTED` and cannot grade its own commit, tree, Git note, or strict result.
...
### Active C3A cumulative-admission maintenance

C3M's earlier “synthetic C3PB receipt phase” heading is retained in its status as historical checker evidence. In the live handoff, C3M and C3PB are closed and C3A is the sole active unit.

The phase classifier still recognizes complete C3M-active and C3PB-active tuples so stale state receives a precise diagnostic, but neither tuple is currently admissible. Both receipt-absent and receipt-present synthetic plans must preserve exactly one complete C3A descendant tuple at its canonical Markdown locations: exactly one exact line for each of four named metadata keys inside `## Current state`, then the matching active subsection heading and receipt-presence paragraph. Missing, duplicated, relocated-to-history, same-line historical embedding, mixed, or coherent whole-tuple downgrade state fails. Canonical C3A receipt markers and grade forms are forbidden across every C3A-owned Markdown file.
...
The C3P receipt declaration and one exact source-receipt block remain closed source evidence. C3PB is independently sealed; C3A changes neither receipt payload nor any C3P/C3PB grade.
...
The phase checker likewise retains the following pre-seal compatibility statement. It is historical evidence only; C1E, C1B, C2, C2M, C2B, C3P, C3V, C3M, and C3PB subsequently sealed, and C3A is now the live unit:
...
1. Run `git status --short --branch`; confirm branch `codex/countershape-autopilot`, sealed C3PB parent `13369122ba7d5557eba1949095c1135a41843070`, tree `d710d9b249786f3fb648f566ae542f1d9ddec180`, and changes limited to C3A's exact thirteen-path roster at `sha256:cc9bb98ceeb2f281336d3e235377d3cdb8920664f833baea830a84b847621188`. Retain stash object `3e766715609680adaef4678fc9aeaa9e3a8bae09` and every earlier stash/failed/superseded ledger by full identity. No `.didrun` or `.didrun-history` path may be staged or tracked.
2. Finish the C3A plan-phase, exact-symbol scanner, symlink-entrypoint, 128/129-byte model, unit-scope, and cumulative-verifier hostile loops. Any red result remains unclaimed and must be fixed rather than bypassed.
3. Archive the zero-claim development ledger; rotate the development-populated `.countershape/p07bc-c3a-final` root; recreate that root plus `home`, `tmp`, `gotmp`, `gocache`, `gopath`, and `gomodcache` as owned mode-`0700` nonsymlink directories. On one exact staged tree, run and immediately claim only C3A's eleven declared commands using the manifest's common `/usr/bin/env -i` prefix, commit as `fix: align pre-C3 architecture and runtime bounds`, seal, inspect the Git note, loop `NO_COLOR=1 didrun verify --strict` to exit `0`, generate the exact-commit HTML report, and archive the final ledger. The chain gate binds exact argv, repository cwd, and stable didrun wrapper fingerprint for the first ten events, then validates its own live Node/invocation/effective bindings and private directories. Event eleven cannot self-prove its outer launcher because didrun appends that event after exit; inspect the recorded event honestly, without treating it as part of the preceding-chain claim. The fingerprint is not a full environment attestation.
4. Only after C3A closes, revalidate and apply stash `3e766715609680adaef4678fc9aeaa9e3a8bae09` without dropping. Any overlap must preserve the sealed C3A repairs, and the post-apply inventory must remain within C3's frozen forty-path roster. Enroll the sealed C3A identity in C3's predecessor manifest, finish the separate inherited-C1 topology transition within C3's frozen checker paths, and resume the full C3 build-loop.
...
5. the current bounded shippable unit: exact thirteen-path C3A `SOURCE_FULL`; sealed C3PB commit `13369122ba7d5557eba1949095c1135a41843070` is the closed receipt prerequisite, the C3P source grades below remain bound only to the independently sealed C3P source, C3A remains `UNRECEIPTED`, C3 is blocked on its closure, and every unrun live/runtime capability remains excluded and `UNRECEIPTED`.
