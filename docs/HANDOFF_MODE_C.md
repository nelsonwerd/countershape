...
- **Pipeline phase:** U0–U6, P07A/U6b, every P07B-A/B boundary, and P07B-C through C3A are sealed and strict-clean. C3L legacy-lattice maintenance is the active exact nine-path `SOURCE_FULL` unit and remains `UNRECEIPTED`; sealed C3A commit `2b84d2841971568784d2ac955775b4a99ca7f0f6`, tree `b8d858033431fe51c262ce725b9d9b051ba9e8c9`, is the closed cumulative-admission prerequisite. C3 stays blocked until C3L independently closes.
- **Git:** repository is on `codex/countershape-autopilot` at sealed C3A HEAD `2b84d2841971568784d2ac955775b4a99ca7f0f6`, tree `b8d858033431fe51c262ce725b9d9b051ba9e8c9`. The dirty tree is limited to C3L's exact nine-path roster at `sha256:b3624e1337681e0909e6c074246103e066118a5924e8f790e7d1f42359da9dc0`; sealed C3A retains its exact thirteen-path roster at `sha256:cc9bb98ceeb2f281336d3e235377d3cdb8920664f833baea830a84b847621188`, and C3 remains frozen at `sha256:c0050817c739e6bf7505105113ebbfd2c2504674f93f14e34daf0609f7fad0fb`. C3 stash objects `fa737661e61c36c10ce793f64852836edec20065` and `3e766715609680adaef4678fc9aeaa9e3a8bae09` remain retained without dropping; older receipt-history stashes remain untouched.
- **didrun:** installed globally and unchanged. C3A's sealed final ledger is `.didrun-history/2026-07-19-p07b-c-c3a-final/.didrun/`; its exact-commit HTML is `.countershape/evidence/p07b-c-c3a-2b84d284.html`. C3L uses a zero-claim development ledger and a separate fresh twelve-claim final ledger; no C3A event is reused. Before final-ledger rotation, any live `.didrun/` is C3L development evidence only; after explicit rotation, the fresh twelve-claim ledger is C3L final evidence pending commit, seal, and strict closure.
- **Current baseline:** from a fresh repository-root shell, run `/opt/homebrew/bin/node tools/verify-current.mjs`. C3L conditionally admits exactly the future C3 store-model edge through U6, preserves the old lattice when that bridge is absent, and then runs U6, B, their hostile self-tests, and the full cumulative verifier through didrun. A direct pass has no didrun grade. The sealed C3P receipt block remains immutable source evidence.
...
### Active C3L legacy-lattice maintenance

C3M's earlier “synthetic C3PB receipt phase” heading is retained in its status as historical checker evidence. The exact phrase “C3M receipt-phase checker maintenance is the active `SOURCE_FULL` unit” is retained here only as inert compatibility history, as is preserved C3PB-era stash object `5eeb848c334ad3a8a44e4cf61fa298e452188ab1`. In the live handoff, C3M, C3PB, and C3A are closed and C3L is the sole active unit.

The phase classifier still recognizes complete C3M-active, C3PB-active, and C3A-active tuples so stale state receives a precise diagnostic, but none is currently admissible. Both receipt-absent and receipt-present synthetic plans must preserve exactly one complete C3L tuple at its canonical Markdown locations: exactly one exact line for each of four named metadata keys inside `## Current state`, then the matching active subsection heading and receipt-presence paragraph. Missing, duplicated, relocated-to-history, same-line historical embedding, HTML-commented, fenced, raw-HTML-wrapped, mixed, or coherent whole-tuple downgrade state fails. Canonical C3L receipt markers and grade forms are forbidden across every C3L-owned Markdown file.
...
The C3P receipt declaration and one exact source-receipt block remain closed source evidence. C3A is independently sealed; C3L changes neither receipt payload nor any C3P/C3PB/C3A grade.
...
The phase checker likewise retains the following pre-seal compatibility statement. It is historical evidence only; C1E, C1B, C2, C2M, C2B, C3P, C3V, C3M, C3PB, and C3A subsequently sealed, and C3L is now the live unit:
...
1. Run `git status --short --branch`; confirm branch `codex/countershape-autopilot`, sealed C3A parent `2b84d2841971568784d2ac955775b4a99ca7f0f6`, tree `b8d858033431fe51c262ce725b9d9b051ba9e8c9`, and changes limited to C3L's exact nine-path roster at `sha256:b3624e1337681e0909e6c074246103e066118a5924e8f790e7d1f42359da9dc0`. Retain stash objects `fa737661e61c36c10ce793f64852836edec20065` and `3e766715609680adaef4678fc9aeaa9e3a8bae09`, plus every earlier stash/failed/superseded ledger, by full identity. No `.didrun` or `.didrun-history` path may be staged or tracked.
2. Finish the C3L U6 conditional-lattice, plan-phase, unit-scope, B-compatibility, and cumulative-verifier hostile loops. Require the exact old lattice when the bridge is absent and the exact four-field/four-method typed bridge when it is present. Any red result remains unclaimed and must be fixed rather than bypassed.
3. Archive the zero-claim development ledger; rotate any development-populated `.countershape/p07bc-c3l-final` root; recreate that root plus `home`, `tmp`, `gotmp`, `gocache`, `gopath`, and `gomodcache` as owned mode-`0700` nonsymlink directories. On one exact staged tree, run and immediately claim only C3L's twelve declared commands using the manifest's common `/usr/bin/env -i` prefix, commit as `fix: admit exact C3 store model edge through U6`, seal, inspect the Git note, loop `NO_COLOR=1 didrun verify --strict` to exit `0`, generate the exact-commit HTML report, and archive the final ledger. The chain gate binds exact argv, repository cwd, and stable didrun wrapper fingerprint for the first eleven events, then validates its own live Node/invocation/effective bindings and private directories. It also binds current `HEAD` to sealed C3A's exact commit/tree and exact `refs/notes/didrun` blob/body/11-claim coverage before the C3L commit. Event twelve cannot self-prove its outer launcher because didrun appends that event after exit; inspect the recorded event honestly, without treating it as part of the preceding-chain claim. The fingerprint is not a full environment attestation.
4. Only after C3L closes, revalidate and apply stash `fa737661e61c36c10ce793f64852836edec20065` without dropping. Any overlap must preserve the sealed C3A/C3L repairs, and the post-apply inventory must remain within C3's frozen forty-path roster. Bind the sealed C3L identity as C3's direct predecessor while retaining sealed C3A as C3L's parent, finish the separate inherited-C1 topology transition within C3's frozen checker paths, and resume the full C3 build-loop.
...
5. the current bounded shippable unit: exact nine-path C3L `SOURCE_FULL`; sealed C3A commit `2b84d2841971568784d2ac955775b4a99ca7f0f6` is the closed cumulative-admission prerequisite, the C3P source grades below remain bound only to the independently sealed C3P source, C3L remains `UNRECEIPTED`, C3 is blocked on its closure, and every unrun live/runtime capability remains excluded and `UNRECEIPTED`.
