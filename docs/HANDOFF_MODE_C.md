...
- **Pipeline phase:** U0–U6, P07A/U6b, every P07B-A/B boundary, and P07B-C through C3 are sealed and strict-clean. C3R note-preview checker maintenance is the active exact seven-path `SOURCE_FULL` unit and remains `UNRECEIPTED`; sealed C3 commit `029c5cf43853eb3cb46520f6e0dea8431a3de5c0`, tree `de3e02a343f8ecfb344bdb28d014342f5b423959`, is the closed source boundary. C3B stays blocked until C3R independently closes.
- **Git:** repository is on `codex/countershape-autopilot` at sealed C3 HEAD `029c5cf43853eb3cb46520f6e0dea8431a3de5c0`, tree `de3e02a343f8ecfb344bdb28d014342f5b423959`. The dirty tree is limited to C3R's exact seven-path roster at `sha256:fe14d9f9985771cce0525edfca47c001b5912dacc4c2d6fef182458d3b44f58e`; the sealed C3 source roster remains `sha256:c0050817c739e6bf7505105113ebbfd2c2504674f93f14e34daf0609f7fad0fb`, and C3B remains frozen at `sha256:3ef17156a2f3051171da70983f9428de2b9bdb5a179602407a36f34200012a67`. C3B draft stash object `fc02f6722716f0600b1e1d7af28cb91925d1047c` and earlier checkpoints remain retained without dropping; older receipt-history stashes remain untouched.
- **didrun:** installed globally and unchanged. C3's sealed final ledger is `.didrun-history/2026-07-19-p07b-c-c3-final/.didrun/`; its exact-commit HTML is `.countershape/evidence/p07b-c-c3-final-029c5cf43853.html`. C3R uses a zero-claim development ledger and a separate fresh nine-claim final ledger; no C3 event is reused. Before final-ledger rotation, any live `.didrun/` is C3R development evidence only; after explicit rotation, the fresh nine-claim ledger is C3R final evidence pending commit, seal, and strict closure.
- **Current baseline:** from a fresh repository-root shell, run `/opt/homebrew/bin/node tools/verify-current.mjs`. C3R admits only one exact didrun note-preview projection for C3 claim eight while preserving byte-exact raw-ledger argv authority, then runs plan/scope self-tests and the full cumulative verifier through didrun. A direct pass has no didrun grade. C3R changes no product, verifier, didrun install, sealed C3 note, or frozen C3B scope.
...
### Active C3R note-preview checker maintenance

C3R is a narrow interposed `DEFECT_REPAIR` after the sealed C3 source and before the frozen C3B receipt-only descendant. didrun's exported Git-note projection replaced only the first long identifier inside C3 claim eight's exact four-test race selector with `«redacted:high-entropy»`; the ignored local ledger retained the complete raw argv. The prior checker required a byte-identical raw tail for every note preview, so it rejected that valid sealed projection. C3R admits one exact sealed-note argv projection for that one claim and no other tail redaction, while preserving byte-exact raw local-ledger argv validation.

The phase classifier recognizes C3 only as a coherent stale predecessor tuple. Receipt-absent plans must preserve exactly one complete C3R tuple at the canonical six Markdown locations. Once the exact C3 receipt declaration and independently sealed C3R authority both exist, all six locations transition atomically to C3B. Missing, duplicated, relocated, inertly embedded, commented, fenced, raw-HTML-wrapped, mixed, or coherent older tuples fail.

C3R operator seal handling is outcome-dependent: run the plain seal first. A successful plain seal must record `secrets_override: false`; only a refusal on reviewed aggregate high-entropy findings after the claimed scoped staged scan permits the operator to use a logged `--allow-secrets` retry that records `secrets_override: true`. Portable C3R authority does not prove plain-seal-first sequencing, refusal, or review: the Git note records only the resulting boolean. When C3B becomes active, its canonical didrun line must render that exact recorded seal disclosure with the same limitation. Neither value proves secret absence, a general credential audit, or publication authority.

Historical classifier fixtures still exercise the exact `relocated-to-history` rejection. Retained predecessor checkpoints include `f7160bbbc09245a1026eb6396ac76282ab913192`, `fa737661e61c36c10ce793f64852836edec20065`, and `3e766715609680adaef4678fc9aeaa9e3a8bae09`; they are inert history and are never applied or dropped by C3R.

The C3B draft is preserved without dropping as stash object `fc02f6722716f0600b1e1d7af28cb91925d1047c`. Its first failed plan event and zero-claim development history are retained permanently at `.didrun-history/2026-07-20-p07b-c-c3b-build-loop-pre-c3r/.didrun/`; that red history supports no C3B or C3R claim. C3R changes no product source, verifier roster, didrun installation, sealed C3 commit/note, or frozen C3B path/claim contract.

The C3P receipt declaration and one exact source-receipt block remain closed source evidence. C3 is independently sealed; active C3R changes neither C3 payload nor any source grade, and no C3 source receipt exists yet.
...
1. Run `git status --short --branch`; confirm branch `codex/countershape-autopilot`, sealed C3 parent `029c5cf43853eb3cb46520f6e0dea8431a3de5c0`, tree `de3e02a343f8ecfb344bdb28d014342f5b423959`, and changes limited to C3R's exact seven-path empty-prefix roster at `sha256:fe14d9f9985771cce0525edfca47c001b5912dacc4c2d6fef182458d3b44f58e`. Retain C3B draft stash `fc02f6722716f0600b1e1d7af28cb91925d1047c`, every earlier checkpoint, and every failed/superseded development ledger by full identity. No `.didrun` or `.didrun-history` path may be staged or tracked.
2. Finish C3R's genuine multi-pass build loop: exact sealed-note projection, raw-local-evidence separation, C3R/C3B phase reconstruction, plan/scope hostile tests, cumulative verifier, and documentation checks. Treat every critic finding and every nonzero didrun event as unresolved until the cause is repaired and the stronger replacement evidence passes; never weaken a roster, claim, or success marker.
3. Archive the zero-claim C3R development ledger; rotate any development-populated `.countershape/p07bc-c3r-final` root; recreate that root plus `home`, `tmp`, `gotmp`, `gocache`, `gopath`, and `gomodcache` as owned mode-`0700` nonsymlink directories. On one exact staged tree, run and immediately claim only C3R's nine declared commands with the common `/usr/bin/env -i` prefix, commit as `fix: validate sealed C3 note previews`, run the plain seal first, and use logged `--allow-secrets` only after plain-seal refusal plus review of the already-claimed scoped scan. Inspect the Git note and preserve its exact recorded `secrets_override: false` or `secrets_override: true` disclosure for C3B's rendered ledger-profile line, without claiming that portable authority proves the operator's sequencing, refusal, or review. Loop `NO_COLOR=1 didrun verify --strict` to exit `0`, generate the exact-commit HTML report, and archive the final ledger. Event nine binds exact sealed C3 authority plus the preceding eight live events and absent seal; it cannot prove its own outer wrapper. The machine-readable unit-scope contract is schema v12.
4. Only after C3R is independently sealed and strict-clean, restore the nonoverlapping C3B draft bytes from stash `fc02f6722716f0600b1e1d7af28cb91925d1047c` without dropping it, deliberately merge the handoff to preserve sealed C3R authority, and create the unchanged exact three-path C3B receipt reconciliation at `sha256:3ef17156a2f3051171da70983f9428de2b9bdb5a179602407a36f34200012a67`. Run and immediately claim its exact seven narrow receipt commands in a fresh ledger, commit, seal, inspect its note, loop strict verification to zero, emit its exact-commit HTML report, and archive the ledger. C3B may reconcile only the sealed C3 source evidence and bind sealed C3R as direct predecessor; it changes no source/verifier behavior and claims no cumulative run, freshness, security, or C4 authority.
...
5. the current bounded shippable unit: exact seven-path C3R `SOURCE_FULL`; sealed C3 commit `029c5cf43853eb3cb46520f6e0dea8431a3de5c0` is the direct source parent, C3's tracked source grades remain unreconciled pending the separate C3B declaration, C3R remains `UNRECEIPTED`, C3B and C4 are blocked on its closure, and every unrun process/runtime capability remains excluded and `UNRECEIPTED`.
