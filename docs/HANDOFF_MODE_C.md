...
- **Pipeline phase:** U0–U6, P07A/U6b, every P07B-A/B boundary, and P07B-C through C2B are sealed and strict-clean. C3P is the active exact 25-path, no-prefix `SOURCE_FULL` runtime-epoch prerequisite and every C3P grade remains `UNRECEIPTED`. C3 stays blocked until C3P, the queued separately scoped verifier-throughput maintenance, and the later exact three-path C3PB receipt reconciliation all close.
- **Git:** repository is on `codex/countershape-autopilot` at sealed C2B HEAD `ab5e2dd5b66702a1d1cd13eb0047b57a48469487`, tree `02fd396858ca41cff6d0ee561dce7ee65a3f94d0`. The dirty tree is the active C3P source work and must converge to its exact 25-path roster digest `sha256:8b047704df047cb96f1c5d19418c4cfe8502e4b4746bb4f24c0851dac8d018c0`. Stash objects `1d2c14f160ff42d509d96219512fa413830a9b1d` and `229a60c0630e296b67faa3c81b91fac73f9f63ff` remain retained; address both by full identity.
- **didrun:** installed globally and unchanged. C2 source/C2M/C2B ledgers remain archived under their dated `.didrun-history/2026-07-19-p07b-c-*` directories. C2B's final ledger is `.didrun-history/2026-07-19-p07b-c-c2b-final/.didrun/`; its exact-commit report is `.countershape/evidence/p07b-c-c2b-final-ab5e2dd5b667.html`. The live `.didrun/` is C3P zero-claim development evidence and currently contains the checked-example regeneration event; it supports no final grade and must be archived before the fresh C3P final ledger.
- **Current baseline:** from a fresh repository-root shell, run `/opt/homebrew/bin/node tools/verify-current.mjs`. The active C3P source boundary must run the complete cumulative profile plus its exact model/planning/scope checks through didrun. A direct pass has no didrun grade. The later C3PB unit is deliberately narrower and may bind source evidence only after C3P independently seals.
...
### C3P prerequisite scope declarations

The sealed C2 source roster remains `sha256:a463e4e9ad9830cb420bbad1da494c26d55e3f68b983abfd683a0c8cb1c3a522`; its sealed C2B receipt roster remains `sha256:5e30e009bb29672d585ece9893bd7c8db198915335a73ded3f3c790ad18f665b`. C3P owns 25 exact source paths at `sha256:8b047704df047cb96f1c5d19418c4cfe8502e4b4746bb4f24c0851dac8d018c0`; the two added verifier paths make the dedicated receipt checker a cumulative gate instead of an orphan. Its later C3PB receipt descendant owns three exact paths at `sha256:531cbe600cf2747752749890ea9a15c3d0126e498da6838d4f6faeb1a0d7e885`. C3 is frozen to forty exact paths and no prefixes at `sha256:c0050817c739e6bf7505105113ebbfd2c2504674f93f14e34daf0609f7fad0fb`; its later C3B receipt descendant owns three exact paths at `sha256:3ef17156a2f3051171da70983f9428de2b9bdb5a179602407a36f34200012a67`. These are scope declarations, not receipts or runtime authority.
...
The phase checker likewise retains the following pre-seal compatibility statement. It is historical evidence only; C1E, C1B, C2, C2M, and C2B later sealed, and C3P is now the live unit:
...
1. Run `git status --short --branch`; confirm branch `codex/countershape-autopilot`, sealed C2B parent `ab5e2dd5b66702a1d1cd13eb0047b57a48469487`, tree `02fd396858ca41cff6d0ee561dce7ee65a3f94d0`, and changes limited to the exact C3P 25-path roster. Retain both stash objects and every failed or superseded ledger. No `.didrun` or `.didrun-history` path may be staged or tracked.
2. Finish the C3P source checker and controlling-doc corrections. Freeze exact profile `DARWIN_KERN_BOOTSESSIONUUID_V1`: parse and lowercase each 36-character nonzero UUID sample, then compare the two canonical values; reject the retired clock profile and every clock, uptime, PID, file, process-local, subprocess, PATH, malformed, unstable, or unsupported fallback. C3P performs no live host measurement.
3. Exercise C3P through a zero-claim build loop, stage exactly its 25 nonzero stage-zero mode-`100644` blobs, then archive that development ledger. In a fresh final ledger run and immediately claim only the twelve declared C3P rows, including the complete cumulative verifier and exact staged source/credential/chain gates.
4. Commit C3P as `fix: use stable Darwin boot-session identity` without an AI co-author trailer. Seal it, independently require its Git note, loop `NO_COLOR=1 didrun verify --strict` to exit `0`, generate the exact-commit HTML report, and archive the complete ledger. C3P's own status cannot self-receipt.
5. At that sealed boundary, route the pending verifier-throughput proposal item-by-item as defect, intentional, or declined-with-reason in a separately scoped maintenance unit; do not mutate didrun itself. Preserve evidence strength, re-receipt any changed load-sensitive policy, and close that unit through the same commit/seal/strict loop before resuming receipt reconciliation.
6. Then create only the exact C3PB three-path receipt snapshot with roster digest `sha256:531cbe600cf2747752749890ea9a15c3d0126e498da6838d4f6faeb1a0d7e885`. Bind the already-sealed C3P note, exact claim map, HTML snapshot, and ignored ledger; label local evidence honestly, run the ordered seven narrow claims, commit/seal/strict-close it, and never grade C3PB from its own text.
7. Resume C3 only after every intervening prerequisite closes. C3 owns the exact forty-path empty-prefix roster, direct materialization from opaque `InspectedTree`, the narrow compiler-frozen store bridge, live Git/runtime/attempt/UUID authority, and sole `OfficialTarget` issuance. C4 alone may issue the one-shot permit, spawn, establish physical chronology, and publish finalized-run semantics. C3B remains a separate receipt boundary before C4.
...
5. the current bounded shippable unit: exact 25-path C3P runtime-epoch semantics plus cumulative receipt-checker enrollment only; sealed C2B and all earlier source/receipt evidence remain historical and unchanged, every C3P grade is `UNRECEIPTED`, C3 is blocked on C3P plus later maintenance and C3PB closure, and every unrun live/runtime capability remains excluded and `UNRECEIPTED`.
