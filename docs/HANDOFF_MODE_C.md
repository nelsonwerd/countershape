...
- **Pipeline phase:** U0–U6, P07A/U6b, every P07B-A/B boundary, and P07B-C through C3B are sealed and strict-clean. C3D data-driven receipt-phase machinery is the active exact seven-path `SOURCE_FULL` defect repair and remains `UNRECEIPTED`; C4 stays blocked until C3D independently closes.
- **Git:** repository is on `codex/countershape-autopilot` at sealed C3B HEAD `3b0b56d2bdd36531caf31e3dbad555aca061616d`, tree `15edcc92c5d1ed79885cf9caaa85b4be36918433`. The dirty tree is limited to C3D's exact seven-path roster at `sha256:ac4436680c612ad206d2786ddd3146af747e1f8ea2bdedf898afca67d46fcd63`; C3B's sealed receipt roster remains `sha256:3ef17156a2f3051171da70983f9428de2b9bdb5a179602407a36f34200012a67`.
- **didrun:** installed globally and unchanged. C3B's sealed final ledger is `.didrun-history/2026-07-20-p07b-c-c3b-final/.didrun/`; its exact-commit HTML is `.countershape/evidence/p07b-c-c3b-final-3b0b56d2bdd3.html`. Its Git note blob is `e447175c54e8fa864f540f09f8bf614962cd8e07`, body SHA-256 `7602ba5baa1e23fc1d309ea4d50c23b843dd87d6f05efe732e8cf656358b2d4b`, and records `secrets_override: true`. C3D uses a zero-claim development ledger and a separate fresh ten-claim final ledger; no C3B event is reused.
- **Current baseline:** from a fresh repository-root shell, run `/opt/homebrew/bin/node tools/verify-current.mjs`. C3D changes checker, schema, and documentation authority only, then runs the complete cumulative verifier profile; it changes no product, verifier roster, execution setting, or didrun installation.
...
The sentence “The active C3 source unit remains `UNRECEIPTED`” is retained only as a historical checker-compatibility anchor. C3 and its later C3B receipt boundary are sealed; C3D is the sole active unit and cannot inherit either boundary's grades.

The phrase “Active C3V throughput-proposal reconciliation” is retained only as a historical checker-compatibility anchor; C3V is not the current unit. C3V routed the observer's four items against sealed C1V commit `88e62acb3023dcd6ee51950c2ad24bbcc2ca8900`, tree `6c98b3c384f01b63d1a00a002303041576896609`, whose `10/10` strict-clean receipt includes the exact 52-case qualification matrix and three cumulative passes at `814384`, `815886`, and `813933` milliseconds. Persistent Go-only cache reuse remains declined for this Darwin/cgo tree; the already-qualified direct `p=2` general plus `p=1` sensitive/nested partition is retained after the `p=4` candidate caused permanent later AST timeouts; the fail-fast O_EXCL lock is already present; and the seven-claim `RECEIPT_RECONCILIATION` profile already supplies the narrow docs-only battery. C3V changes only its seven declared documentation/checker-authority paths and does not edit the verifier or repetition runner. C3L and C3F do not reopen those dispositions.
...
The phase checker likewise retains the following pre-seal compatibility statement. It is historical evidence only; C1E, C1B, C2, C2M, C2B, C3P, C3V, C3M, C3PB, C3A, C3L, C3F, C3S, C3, C3R, C3Q, C3T, C3U, and C3B subsequently sealed, and C3D is now the sole active source-maintenance unit:
...
1. Run `git status --short --branch`; confirm branch `codex/countershape-autopilot`, sealed C3B commit `3b0b56d2bdd36531caf31e3dbad555aca061616d` remains `HEAD`, and changes are limited to C3D's exact seven-path empty-prefix roster at `sha256:ac4436680c612ad206d2786ddd3146af747e1f8ea2bdedf898afca67d46fcd63`. Retain every named stash, checkpoint, final ledger archive, and failed/superseded development ledger by full identity. No `.didrun` or `.didrun-history` path may be staged or tracked.
2. Finish C3D's multi-pass source-maintenance loop: schema/table invariants, generated fixture catalog, fixed C3R/C3Q/C3T/C3U regressions, sealed-C3B note compatibility, unit-scope self-test, cumulative verifier self-test, full cumulative verification, exact staged scope, structured credential scan, and predecessor/chain authority. Treat every critic finding and every nonzero didrun event as unresolved until the real cause is repaired and stronger replacement evidence passes.
3. Generate `--print-final-runbook C3D` and follow that deterministic instruction artifact. Every shell fence runs in its own `set -eu` subshell, independently enters and asserts the physical canonical repository root, and stops before its claim or later mutation after any failure, so fences may be executed separately without inherited cwd or variables; post-commit fences re-derive current immutable HEAD and its exact first 12 characters under sole-writer control. The runbook projects the exact checker-owned `/usr/bin/env -i` argv and immediate claims, refuses existing final roots, prepares mode-`0700` `.countershape/evidence`, emits no staging or deletion, fixes the commit subject, requires the plain seal first, leaves any `--allow-secrets` choice at an explicit reviewed manual checkpoint, inspects the note, treats a nonzero strict result as a fresh-boundary repair rather than an unchanged verify loop, generates exact-commit HTML only after strict succeeds, and copies the ledger to its canonical commit-addressed ignored archive while retaining live evidence. `--print-final-runbook <C3D|C6A|C6M|C6B>` confers no grade; C4 and C5 reject as topology-only until their owning contracts exist.
4. Do not extend the frozen pre-capsule lifecycle adapters for C4 or C5. Advance the active phase by changing the owning boundary's staged HANDOFF capsule and running its explicit `--check-candidate-phase <unit>` gate; never edit the C3D-sealed topology table, and never infer the active boundary from receipt state. C4, C5, C6A, C6M, and C6B do not own `spec/verification/p07b-c-unit-paths.json`. C6M introduces only the canonical sealed-C6A source-authority manifest, owns neither checker, and keeps both receipt projections absent; C6B alone inherits that manifest unchanged and materializes both canonical projections. Preserve the exact C3R/C3Q/C3T/C3U regressions, 1,320 historical rejections, 440 accepted controls, eight aliases, two strict-zero controls, and terminal C3 bytes.
5. After C3D independently closes, resume C4. C3 owns only the inert pre-spawn target; C4 alone may issue the one-shot permit, spawn, establish physical chronology, and publish finalized-run semantics. Preserve the explicit nonclaims around Node authenticity, coordinated same-user substitution, physical reboot/survivor absence, containment, confidentiality, adoption, and production readiness.
...
5. the current bounded shippable unit: exact seven-path C3D `SOURCE_FULL` `DEFECT_REPAIR`; sealed C3B is the direct parent/current preseal `HEAD`, both source-receipt families remain present, every C3D grade is `UNRECEIPTED`, C4 is blocked on C3D closure, and every unrun process/runtime capability remains excluded and `UNRECEIPTED`.
