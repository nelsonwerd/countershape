...
- **Pipeline phase:** U0–U6, P07A/U6b, every P07B-A/B boundary, and P07B-C through C3P are sealed and strict-clean. C3V is the active exact seven-path, no-prefix `SOURCE_FULL` verification-throughput proposal reconciliation; every C3V grade remains `UNRECEIPTED`. C3 stays blocked until C3V and the later exact three-path C3PB receipt reconciliation close.
- **Git:** repository is on `codex/countershape-autopilot` at sealed C3P HEAD `f7b6e6bda7a8864969415ab8636c495902e78dd9`, tree `2d5555db63d46c837cf4e12f2dab2d04a41f4654`. The dirty tree is the active C3V documentation/checker declaration and must converge to its exact seven-path roster digest `sha256:6cc5d8fe929f0f1dfa6ffc223d2e8bcee5ba4596a09caa9dc032103bbb5bafbd`. Stash objects `1d2c14f160ff42d509d96219512fa413830a9b1d` and `229a60c0630e296b67faa3c81b91fac73f9f63ff` remain retained; address both by full identity.
- **didrun:** installed globally and unchanged. C3P development evidence is archived at `.didrun-history/2026-07-19-p07b-c-c3p-build-loop/.didrun/`; its sealed final ledger is `.didrun-history/2026-07-19-p07b-c-c3p-final/.didrun/`. The exact-commit C3P report is `.countershape/evidence/p07b-c-c3p-final-f7b6e6bda7a8.html`. C3V starts with a zero-claim development ledger; no C3P event is reused.
- **Current baseline:** from a fresh repository-root shell, run `/opt/homebrew/bin/node tools/verify-current.mjs`. C3V keeps that implementation byte-for-byte unchanged and must run the complete cumulative profile plus planning/scope self-tests through didrun. A direct pass has no didrun grade. The later C3PB unit is deliberately narrower and may bind source evidence only after both C3P and C3V independently seal.
...
### C3P/C3V prerequisite scope declarations

Sealed C2B commit `ab5e2dd5b66702a1d1cd13eb0047b57a48469487`, tree `02fd396858ca41cff6d0ee561dce7ee65a3f94d0`, remains the reconciled C2 prerequisite. The sealed C2 source roster remains `sha256:a463e4e9ad9830cb420bbad1da494c26d55e3f68b983abfd683a0c8cb1c3a522`; its sealed C2B receipt roster remains `sha256:5e30e009bb29672d585ece9893bd7c8db198915335a73ded3f3c790ad18f665b`. C3P owns 25 exact source paths at `sha256:8b047704df047cb96f1c5d19418c4cfe8502e4b4746bb4f24c0851dac8d018c0`; the two added verifier paths make the dedicated receipt checker a cumulative gate instead of an orphan. C3V owns seven exact source paths at `sha256:6cc5d8fe929f0f1dfa6ffc223d2e8bcee5ba4596a09caa9dc032103bbb5bafbd` and changes no verifier behavior. The later C3PB receipt descendant owns three exact paths at `sha256:531cbe600cf2747752749890ea9a15c3d0126e498da6838d4f6faeb1a0d7e885`. C3 is frozen to forty exact paths and no prefixes at `sha256:c0050817c739e6bf7505105113ebbfd2c2504674f93f14e34daf0609f7fad0fb`; its later C3B receipt descendant owns three exact paths at `sha256:3ef17156a2f3051171da70983f9428de2b9bdb5a179602407a36f34200012a67`. These are scope declarations, not receipts or runtime authority.
...
The phase checker likewise retains the following pre-seal compatibility statement. It is historical evidence only; C1E, C1B, C2, C2M, C2B, and C3P later sealed, and C3V is now the live unit:
...
1. Run `git status --short --branch`; confirm branch `codex/countershape-autopilot`, sealed C3P parent `f7b6e6bda7a8864969415ab8636c495902e78dd9`, tree `2d5555db63d46c837cf4e12f2dab2d04a41f4654`, and changes limited to the exact C3V seven-path roster. Retain both stash objects and every failed or superseded ledger. No `.didrun` or `.didrun-history` path may be staged or tracked.
2. Finish C3V as a no-behavior-change proposal reconciliation. Preserve fresh per-run `GOCACHE`, the qualified direct `p=2` general and serialized sensitive/nested lanes, the fail-fast O_EXCL lock with review-first stale handling, and the explicit seven-claim receipt profile. Route all four proposal items verbatim in the C3V status and do not edit the verifier, repetition runner, product, or didrun.
3. Exercise C3V through a zero-claim build loop, stage exactly its seven nonzero stage-zero mode-`100644` blobs, then archive that development ledger. In a fresh final ledger run and immediately claim only the eight declared C3V rows, including the complete cumulative verifier and exact staged source/credential/chain gates.
4. Commit C3V as `docs: reconcile verifier throughput proposal` without an AI co-author trailer. Seal it, independently require its Git note, loop `NO_COLOR=1 didrun verify --strict` to exit `0`, generate the exact-commit HTML report, and archive the complete ledger. Its pre-seal status cannot self-grade.
5. Then create only the exact C3PB three-path receipt snapshot with roster digest `sha256:531cbe600cf2747752749890ea9a15c3d0126e498da6838d4f6faeb1a0d7e885`. Bind the already-sealed C3P note, exact claim map, HTML snapshot, and ignored ledger; label local evidence honestly, run the ordered seven narrow claims, commit/seal/strict-close it, and never grade C3PB from its own text.
...
5. the current bounded shippable unit: exact seven-path C3V throughput-proposal reconciliation with no verifier behavior change; sealed C3P and all earlier source/receipt evidence remain historical and unchanged, every C3P source grade and every C3V grade is `UNRECEIPTED`, C3 is blocked on C3V plus later C3PB closure, and every unrun live/runtime capability remains excluded and `UNRECEIPTED`.
