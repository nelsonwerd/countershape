...
- **Pipeline phase:** U0–U6, P07A/U6b, every P07B-A/B boundary, and P07B-C through C3Q are sealed and strict-clean. C3T phase-transition self-test maintenance is the active exact seven-path `SOURCE_FULL` unit and remains `UNRECEIPTED`; sealed C3Q commit `818ed90ebbf99ae38916307d9f2342921d38ebbf`, tree `157241ac2ea7c390b2f67a788b26b9f7666b2893`, is the closed checker prerequisite. C3B stays blocked until C3T independently closes.
- **Git:** repository is on `codex/countershape-autopilot` at sealed C3Q HEAD `818ed90ebbf99ae38916307d9f2342921d38ebbf`, tree `157241ac2ea7c390b2f67a788b26b9f7666b2893`. The dirty tree is limited to C3T's exact seven-path roster at `sha256:496788860db6c42f5acc50de0de02be8a63bcb8e11f99fb24d59d9c0e016477e`; sealed C3Q retains its exact seven-path roster at `sha256:f5f27e661c1ace6516aba4b9d5766f4c6ab50b6602dbcb101bff9b6a4ebfe6fc`, and C3B remains frozen at `sha256:3ef17156a2f3051171da70983f9428de2b9bdb5a179602407a36f34200012a67`. Post-C3Q C3B draft stash object `eab9839c902abf52e9729144a2244740228a41eb` and earlier checkpoints remain retained without dropping; the permanent red post-C3Q C3B rehearsal ledger remains archived.
- **didrun:** installed globally and unchanged. C3Q's sealed final ledger is `.didrun-history/2026-07-20-p07b-c-c3q-final/.didrun/`; its exact-commit HTML is `.countershape/evidence/p07b-c-c3q-final-818ed90ebbf9.html`. C3T uses a zero-claim development ledger and a separate fresh nine-claim final ledger; no C3Q event is reused. Before final-ledger rotation, any live `.didrun/` is C3T development evidence only; after explicit rotation, the fresh nine-claim ledger is C3T final evidence pending commit, seal, and strict closure.
- **Current baseline:** from a fresh repository-root shell, run `/opt/homebrew/bin/node tools/verify-current.mjs`. C3T reconstructs one validated receipt-absent C3T fixture before every hostile receipt mutation, proves absent-to-present and present-to-absent-to-present byte stability, and then runs plan/scope self-tests plus the full cumulative verifier through didrun. A direct pass has no didrun grade. C3T changes no product, verifier roster, didrun install, sealed C3/C3R/C3Q evidence, or frozen C3B scope.
...
The sentence “The active C3 source unit remains `UNRECEIPTED`” is retained only as a historical checker-compatibility anchor. C3 itself is sealed; its source grades remain unreconciled until the separate C3B receipt, and active C3T cannot grade its own commit, tree, Git note, or strict result.
...
C3T is the active exact seven-path `SOURCE_FULL` defect repair at `sha256:496788860db6c42f5acc50de0de02be8a63bcb8e11f99fb24d59d9c0e016477e` and remains `UNRECEIPTED`. Its direct parent is sealed C3Q commit `818ed90ebbf99ae38916307d9f2342921d38ebbf`, tree `157241ac2ea7c390b2f67a788b26b9f7666b2893`; C3B's exact three-path roster, receipt profile, and seven labels/types remain frozen.

Sealed C3Q's note blob is `06cd81c05ce2e6f2348d0765298846ac048caf7e` and its note-body SHA-256 is `b61f53351614361f3646186827ed96e7aace8980bcd3be52f470a366a59fe855`. Current C3T/C3B refusal authority covers 43 exact Markdown paths at `sha256:52e78638d70f4e229e0b54c0a8311b6e0d1d2acc0d4e439cacf10cde65c62959`: 2 phases × 43 paths × 15 positive forms, or 1,290 required rejections, plus 430 accepted controls.
...
The C3P receipt declaration and one exact source-receipt block remain closed source evidence. C3, C3R, and C3Q are independently sealed; active C3T changes neither C3 payload nor any source grade, and no C3 source receipt exists yet.
...
1. Run `git status --short --branch`; confirm branch `codex/countershape-autopilot`, sealed C3Q parent `818ed90ebbf99ae38916307d9f2342921d38ebbf`, tree `157241ac2ea7c390b2f67a788b26b9f7666b2893`, and changes limited to C3T's exact seven-path empty-prefix roster at `sha256:496788860db6c42f5acc50de0de02be8a63bcb8e11f99fb24d59d9c0e016477e`. Retain post-C3Q C3B draft stash `eab9839c902abf52e9729144a2244740228a41eb`, earlier stashes `1d677db4b9049084ea066263b1bb6c25ab62e5f0` and `fc02f6722716f0600b1e1d7af28cb91925d1047c`, every earlier checkpoint, and every failed/superseded development ledger by full identity. No `.didrun` or `.didrun-history` path may be staged or tracked.
2. Finish C3T's genuine multi-pass build loop: exact sealed-C3Q authority, a validated `C3T_ACTIVE` absent fixture, exact `C3B_ACTIVE` forward rendering, present → absent → present byte stability, plan/scope hostile tests, cumulative verifier, and documentation checks. Treat every critic finding and every nonzero didrun event as unresolved until the cause is repaired and stronger replacement evidence passes; never weaken a roster, claim, success marker, or permanent failed receipt.
3. Archive the zero-claim C3T development ledger; rotate any development-populated `.countershape/p07bc-c3t-final` root; recreate that root plus `home`, `tmp`, `gotmp`, `gocache`, `gopath`, and `gomodcache` as owned mode-`0700` nonsymlink directories. On one exact staged tree, run and immediately claim only C3T's nine declared commands with the common `/usr/bin/env -i` prefix, commit as `fix: make C3 receipt self-test phase-stable`, run the plain seal first, and use logged `--allow-secrets` only after plain-seal refusal plus review of the already-claimed scoped scan. Inspect the Git note, loop `NO_COLOR=1 didrun verify --strict` to exit `0`, generate the exact-commit HTML report, and archive the final ledger. Event nine binds exact sealed C3Q authority plus the preceding eight live events and absent seal; it cannot prove its own outer wrapper. The machine-readable unit-scope contract is schema v14.
4. Only after C3T independently closes, restore the frozen exact-three-path C3B draft from stash `eab9839c902abf52e9729144a2244740228a41eb` without dropping it, then deliberately merge the handoff so sealed C3T remains the immediate predecessor rather than restoring stale lifecycle bytes wholesale. Keep C3B at `sha256:3ef17156a2f3051171da70983f9428de2b9bdb5a179602407a36f34200012a67`. Run and immediately claim its exact seven narrow receipt commands in a fresh ledger, commit, seal, inspect its note, loop strict verification to zero, emit its exact-commit HTML report, and archive the ledger. C3B may reconcile only sealed C3 source evidence; it must retain exact C3R/C3Q ancestry and bind sealed C3T as direct predecessor/current `HEAD`. It changes no source/verifier behavior and claims no cumulative run, freshness, security, or C4 authority.
...
5. the current bounded shippable unit: exact seven-path C3T `SOURCE_FULL`; sealed C3Q commit `818ed90ebbf99ae38916307d9f2342921d38ebbf` is the direct checker parent, C3's tracked source grades remain unreconciled pending the separate C3B declaration, C3T remains `UNRECEIPTED`, C3B and C4 are blocked on its closure, and every unrun process/runtime capability remains excluded and `UNRECEIPTED`.
