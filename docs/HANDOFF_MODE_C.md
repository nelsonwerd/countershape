...
- **Pipeline phase:** U0–U6, P07A/U6b, every P07B-A/B boundary, and P07B-C through C3T are sealed and strict-clean. C3U cross-phase receipt-fixture maintenance is the active exact seven-path `SOURCE_FULL` unit and remains `UNRECEIPTED`; sealed C3T commit `9d8430aa70b1536a85dd7352a3d339667f2c1e88`, tree `3be4d7cde4d043b87438506483ffb8c256719df2`, is the closed checker prerequisite. C3B stays blocked until C3U independently closes.
- **Git:** repository is on `codex/countershape-autopilot` at sealed C3T HEAD `9d8430aa70b1536a85dd7352a3d339667f2c1e88`, tree `3be4d7cde4d043b87438506483ffb8c256719df2`. The dirty tree is limited to C3U's exact seven-path roster at `sha256:7af7456929b0db08abc2a31bd1cb62d84002c1a6bcdda3f4a8b4896f6b8c30cd`; sealed C3T retains its exact seven-path roster at `sha256:496788860db6c42f5acc50de0de02be8a63bcb8e11f99fb24d59d9c0e016477e`, and C3B remains frozen at `sha256:3ef17156a2f3051171da70983f9428de2b9bdb5a179602407a36f34200012a67`. Canonical post-C3T C3B draft stash object `cca3045b55ddc00e011c4331f4731bd1a433ea36` and earlier checkpoints remain retained without dropping; the permanent post-C3T terminal-block red ledger remains archived.
- **didrun:** installed globally and unchanged. C3T's sealed final ledger is `.didrun-history/2026-07-20-p07b-c-c3t-final/.didrun/`; its exact-commit HTML is `.countershape/evidence/p07b-c-c3t-final-9d8430aa70b1.html`. C3U uses a zero-claim development ledger and a separate fresh nine-claim final ledger; no C3T event is reused. Before final-ledger rotation, any live `.didrun/` is C3U development evidence only; after explicit rotation, the fresh nine-claim ledger is C3U final evidence pending commit, seal, and strict closure.
- **Current baseline:** from a fresh repository-root shell, run `/opt/homebrew/bin/node tools/verify-current.mjs`. C3U inserts every synthetic legacy receipt block before an optional exact terminal C3 block, preserves that C3 authority byte-for-byte, and runs the plan/scope self-tests plus the full cumulative verifier through didrun. A direct pass has no didrun grade. C3U changes no product, verifier roster, didrun install, sealed C3/C3R/C3Q/C3T evidence, or frozen C3B scope.
...
The sentence “The active C3 source unit remains `UNRECEIPTED`” is retained only as a historical checker-compatibility anchor. C3 itself is sealed; its source grades remain unreconciled until the separate C3B receipt, and active C3U cannot grade its own commit, tree, Git note, or strict result.
...
C3U is the active exact seven-path `SOURCE_FULL` defect repair at `sha256:7af7456929b0db08abc2a31bd1cb62d84002c1a6bcdda3f4a8b4896f6b8c30cd` and remains `UNRECEIPTED`. Its direct parent is sealed C3T commit `9d8430aa70b1536a85dd7352a3d339667f2c1e88`, tree `3be4d7cde4d043b87438506483ffb8c256719df2`; C3B's exact three-path roster, receipt profile, and seven labels/types remain frozen.

Sealed C3T's note blob is `d9e371310b360b183d890ed401cb36bd877becda` and its note-body SHA-256 is `05914bf85ce1d3bc48f5debf902babb526faf2500af318c35f08ffdf071ff735`. Current C3U/C3B refusal authority covers 44 exact Markdown paths at `sha256:1c5bf120f1ab948586bfa594e6ae634fc6acf7a5544febcccb2c4dc475a59a49`: 2 phases × 44 paths × 15 positive forms, or 1,320 required rejections, plus 440 core accepted controls; eight supplemental natural aliases reject and two explicit strict-zero prohibition controls remain accepted.
...
The C3P receipt declaration and one exact source-receipt block remain closed source evidence. C3, C3R, C3Q, and C3T are independently sealed; active C3U changes neither C3 payload nor any source grade, and no C3 source receipt exists yet.
...
1. Run `git status --short --branch`; confirm branch `codex/countershape-autopilot`, sealed C3T parent `9d8430aa70b1536a85dd7352a3d339667f2c1e88`, tree `3be4d7cde4d043b87438506483ffb8c256719df2`, and changes limited to C3U's exact seven-path empty-prefix roster at `sha256:7af7456929b0db08abc2a31bd1cb62d84002c1a6bcdda3f4a8b4896f6b8c30cd`. Retain canonical post-C3T C3B draft stash `cca3045b55ddc00e011c4331f4731bd1a433ea36`, earlier stashes `eab9839c902abf52e9729144a2244740228a41eb`, `1d677db4b9049084ea066263b1bb6c25ab62e5f0`, and `fc02f6722716f0600b1e1d7af28cb91925d1047c`, every earlier checkpoint, and every failed/superseded development ledger by full identity. No `.didrun` or `.didrun-history` path may be staged or tracked.
2. Finish C3U's genuine multi-pass build loop: exact sealed-C3T authority, marker-free legacy receipt behavior, optional terminal-C3 partition and byte-preserving reattachment, C0/C1/C2/C3P caller coverage, C2 and C3P inverse recovery, full-plan composition, plan/scope hostile tests, cumulative verifier, and documentation checks. Treat every critic finding and every nonzero didrun event as unresolved until the cause is repaired and stronger replacement evidence passes; never weaken a roster, claim, success marker, or permanent failed receipt.
3. Archive the zero-claim C3U development ledger; rotate any development-populated `.countershape/p07bc-c3u-final` root; recreate that root plus `home`, `tmp`, `gotmp`, `gocache`, `gopath`, and `gomodcache` as owned mode-`0700` nonsymlink directories. On one exact staged tree, run and immediately claim only C3U's nine declared commands with the common `/usr/bin/env -i` prefix, commit as `fix: preserve terminal C3 receipts in legacy fixtures`, run the plain seal first, and use logged `--allow-secrets` only after plain-seal refusal plus review of the already-claimed scoped scan. Inspect the Git note, loop `NO_COLOR=1 didrun verify --strict` to exit `0`, generate the exact-commit HTML report, and archive the final ledger. Event nine binds exact sealed C3T authority plus the preceding eight live events and absent seal; it cannot prove its own outer wrapper. The machine-readable unit-scope contract is schema v15.
4. Only after C3U independently closes, restore the frozen exact-three-path C3B draft from stash `cca3045b55ddc00e011c4331f4731bd1a433ea36` without dropping it, then deliberately merge the handoff so sealed C3U remains the immediate predecessor rather than restoring stale lifecycle bytes wholesale. Keep C3B at `sha256:3ef17156a2f3051171da70983f9428de2b9bdb5a179602407a36f34200012a67`. Run and immediately claim its exact seven narrow receipt commands in a fresh ledger, commit, seal, inspect its note, loop strict verification to zero, emit its exact-commit HTML report, and archive the ledger. C3B may reconcile only sealed C3 source evidence; it must retain exact C3R/C3Q/C3T ancestry and bind sealed C3U as direct predecessor/current `HEAD`. It changes no source/verifier behavior and claims no cumulative run, freshness, security, or C4 authority.
...
5. the current bounded shippable unit: exact seven-path C3U `SOURCE_FULL`; sealed C3T commit `9d8430aa70b1536a85dd7352a3d339667f2c1e88` is the direct checker parent, C3's tracked source grades remain unreconciled pending the separate C3B declaration, C3U remains `UNRECEIPTED`, C3B and C4 are blocked on its closure, and every unrun process/runtime capability remains excluded and `UNRECEIPTED`.
