...
`internal/compare` remains the authority for the canonical `OutcomeMap` and its digest. The reducer consumes that typed digest; it must not reconstruct maps from display clusters. CLI and HTTP keep their own typed stimuli, well-founded measures, and deterministic neighbor functions. `internal/reduce` owns only search order, budgets, tri-valued orchestration, transcript construction, and final-sweep certification. It must not import Git, process, HTTP, CLI, store, or server implementations.

An evaluation is exactly `PRESERVES`, `CHANGES`, or `UNRESOLVED`. `PRESERVES` requires a new complete eligible batch whose full map equals the baseline. `CHANGES` requires a different complete observed-stable eligible map. Every unstable, uncomparable, incomplete, cancelled, timed-out, stale, or budget-precluded result is `UNRESOLVED`. Never encode this as a Boolean.
...
Primary ownership is `internal/reduce/**` and its tests. Add typed neighbor implementations and tests only under `internal/adapters/cli/**` and `internal/adapters/http/**`. Add reusable generated fixtures under `testkit/**` when necessary. Pure compare/domain changes are permitted only to expose an already-specified typed value; do not create a parallel digest or generic JSON stimulus. Do not touch the store, contract emitter, server, or web UI in this unit.
...
4. Implement the final direct-neighbor sweep as a durable certificate input, not a Boolean flag. `ONE_MINIMAL_UNDER` must be constructible only when the enumerated valid neighbor set equals the set of fresh completed `CHANGES` records, with no unresolved, omitted, duplicate, stale, or post-budget member. An empty direct-neighbor set may certify only if enumeration itself completed durably.
