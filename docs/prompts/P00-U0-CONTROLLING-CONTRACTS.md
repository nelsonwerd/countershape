...
- Git selection, materialized bytes, deterministic `WorldPlan`, opaque candidate binding, structural `WorldInstance`, later `InstanceMeasurements`, comparison assessment/admission, captured evidence, projection, human ruling, emitted source, and current conformance are separate jurisdictions.
