...
- Git selection, materialized bytes, deterministic `WorldPlan`, opaque candidate binding, structural historical `WorldInstance`, later `InstanceMeasurements`, comparison assessment/admission, captured evidence, projection, human ruling, emitted source, capability-only later execution target, and final conformance result are separate jurisdictions.
...
Schemas/examples must cover at least `WorldPlan`, `CandidateExecutionBinding`, structural `WorldInstance`, `InstanceMeasurements`, `ComparisonEnvelope`, admitted/rejected comparison, `CapturedObservation`, `StableBatch`, `CandidateOutcomeMap`, `ReductionRun`, `Choicepoint`, `DecisionRecord`, `ContractBundle`, `ContractExecutionTarget`, `FinalizedContractRun`, and `ContractExecution`. Examples are inert contract examples, not proof that runtime canonicalization or capability issuance exists. Include malformed/refusal fixtures: key-only allocation, mismatched plan/binding, rejected-comparison-to-batch, mismatched admission sets, duplicate global evidence, ordinal group identity, control-as-outcome, naked-digest preservation, `UNRESOLVED` one-minimality, reused confirmation evidence, empty fields, allow-many cross-product, noncompilable emission, copied target authority, and target/run mismatch.
