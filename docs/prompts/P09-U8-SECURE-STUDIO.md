...
Use separate blind and reveal DTOs. A blind response must not contain candidate ref/name, model/provider/producer provenance, total candidate count, per-outcome support count, hidden branch data, or a reversible cross-study alias. Distinct outcome count is necessarily visible. Derive Choicepoint-scoped aliases and deterministic fingerprint-only ordering independent of candidate order and support. Identity must be absent at the serialized-byte level, not hidden by CSS. Treat caller-authored scenario text and behavior content as potentially identity-revealing even when system-supplied metadata is absent. Reveal only after recording the provisional action and field acknowledgements; record early reveal and any post-reveal change plus rationale.
...
- Choicepoint/store: render the implemented `CHOICEPOINT_READY` and `RULING` states exactly. Future `DISCOVERED`, deferred reopening, successor-study `REFINE`, `STALE`, and `INVALIDATED` workflows remain disabled and visibly unavailable until their durable authorities exist.
