...
Historical execution contract: C4I is sealed; this section preserves its exact pre-seal instructions and does not make C4I current.
...
- Historically, evolve only the phase authority to `countershape/p07b-c-unit-paths/v22` with 24 ordered phase rows. C4I was active; C5 remained blocked until C4I sealed.
...
Authority is `OWNER_OUT_OF_BAND`. Sealed C4K predeclared restoration of the exact checkpoint in prose, while v23 did not machine-declare C4J: `predecessor-predeclared = true (sealed prose only)` and `machine-predeclared = false`. No external signed authorization artifact exists, and no C4K grade is inherited or rewritten.
