...
- The proof fixture has no setup command or package install. Declarative seed files are written into each fresh world before start. The service signals readiness over a dedicated inherited pipe with the U4-locked `ONE_BYTE_0X01_THEN_EOF_V1` protocol: exactly byte `0x01`, then EOF, within the readiness budget. Empty EOF, a different byte, any extra byte, missing EOF, timeout, cancellation, or child exit is not readiness. Readiness is not an HTTP request and cannot warm application state. U0 named `ready-byte` but did not define its wire bytes; this paragraph is the controlling correction produced by the U4 deep-dive.
...
- Capture transport-versus-response separately. Any complete status—including `500`—may be eligible behavior. Start/readiness/transport/timeout/output/protocol/projection/orphan/teardown failures are ineligible controls and cannot become status codes or outcome cards. A response followed by a natural nonzero or signaled service exit is `PROBE_TRANSPORT_ERROR`; only owner-issued TERM/KILL remains a cleanup fact rather than a natural-exit control.
- Captured evidence retains the volatile request ID and scratch-root value. A visible, versioned projection validates each declared volatile field and then omits it from the fixed output tuple while preserving status and disclosure-bearing metadata. Validate-then-omit proves only projected equality, never that root/port differences did not change earlier control flow.
- Reuse U3’s rotated scheduler and U1’s complete `CandidateOutcomeMap`, including its shared admission set and global evidence-identity checks. No cache, shared-world mode, shape-only summary, or adapter-local classifier exists in product code.
...
It may add one private HTTP-specific service sequencer to `internal/world` and one thin concrete `ExecuteHTTP` world edge that imports only the cycle-free `internal/adapters/http/model` authority. That edge owns allocation, materialization, inherited descriptors, phase transitions, physical exchange, and receipts; it is neither neutral nor an exported arbitrary callback runner. It must preserve the U2/U3 one-shot path unchanged and must not add HTTP switches or fields to `internal/domain`, `internal/observe`, `internal/compare`, `internal/reduce`, or `internal/choice`. Do not create `cmd/`, store, emitter/Node harness, server, report, or web code.
...
2. Compile the HTTP inert spec into deterministic `WorldPlan`. Allocate structural `WorldInstance` only from the opaque candidate binding plus actual matching plan. Keep actual port, roots, process facts, tool/platform observations, lifecycle, and readiness receipt in `InstanceMeasurements` or U2 process receipts. Allocate loopback resources only per attempt. Make the comparison envelope list measured, required-equal, tolerated, rejected, and uncontrolled dimensions explicitly; persist one exact admitted matrix per repetition or stop before batching on rejection.
3. Add fixture-owned readiness via a dedicated inherited descriptor/pipe. Accept exactly U4's `ONE_BYTE_0X01_THEN_EOF_V1` protocol, close it after success, and classify empty EOF, malformed or extra bytes, missing EOF, timeout, cancellation, and process exit without readiness as controls. Do not probe the service to decide readiness.
...
5. Finalize every started attempt as typed captured-or-controlled evidence, including no-response control paths. Only a complete response, exact validated invocation evidence, and clean teardown may become eligible for visible projection. Run projection operations through strict canonical parsing and extraction. Store the pre-projection captured body privately and link every projected value to its exact operation/source.
...
7. Build four dependency-free Node-core candidates: A observed-stable `403`; B observed-stable `404`; C observed-stable metadata-bearing `200`; D alternates `404`/`200` from the bound schedule repetition. Every response includes volatile request ID and scratch root. External fixture instrumentation proves exactly one request and exact request bytes, but is not authority for D's alternation; it is excluded from selected projection.
8. Make the declared request causal before candidate-specific behavior: exact method/path; ordered query members including tenant, role, audit flag, and duplicate tags; required tenant/role/audit and ordered trace/tag headers; present-empty header; and absent body must all drive application policy. Add a second typed tenantless seed shape that keeps request bytes and the eligible A/B/C roster identical while changing the physical map from `403|404|200` to `200|500|500`.
9. Build a deliberately naive **test-only** shared-root runner whose state contamination makes B and C appear equal. Label its result `NEGATIVE_FIXTURE_NON_PRODUCT`; ensure no production API can enable shared roots or import this helper. Then prove fresh product worlds restore the actual B/C split.
10. Expose a test-readable Captured-to-Projection transcript. Mutating the projection to remove status or disclosure-bearing metadata must change/fail the expected map; volatile-only removal must preserve it.
11. Update status/handoff with the observed baseline, negative fixture meaning, exact readiness/wire profile, request-causality and seed-shape results, and unresolved proof work for U5+.
...
Architecture checks must prove generic packages contain no HTTP imports/field names/type switches, CLI types remain unchanged and typed, and only one canonical outcome-map digest reaches study output. The mutation driver must anchor and kill all sixteen locked cases: readiness-by-HTTP request; shared-root product option; proxy environment use; redirect following; status `500` as infrastructure failure; transport failure as status; teardown failure admitted; volatile removal hidden from transcript; status removal accepted; disclosure metadata removal accepted; D classified by majority; map comparison reduced to group shape; absent body collapsed into present-empty; content-length mismatch accepted; wrong readiness byte accepted; and invocation request digest ignored. Missing anchors, missing physical killers, or an open Go/non-Go sandbox closure are failures.
