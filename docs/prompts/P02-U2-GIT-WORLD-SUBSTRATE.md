...
Read completely: `/Users/drewnelson/.claude/CLAUDE.md`; `docs/CONCEPT_BRIEF.md`; `docs/ARCHITECTURE.md`; `docs/SEMANTICS.md`; `docs/PROJECTION_ALGEBRA.md`; `docs/THREAT_MODEL.md`; `docs/STATE_MACHINES.md`; `docs/CLAIM_VOCABULARY.md`; `research/deep-dive/02-architecture-correctness.md`; `research/deep-dive/03-execution-security.md`; `research/deep-dive/07-SYNTHESIS.md`; `research/deep-dive/08-RED_TEAM.md`; `docs/status/U0.md`; `docs/status/U1.md`; `docs/HANDOFF_MODE_C.md`; and all schemas/vectors relevant to candidate binding, `WorldPlan`, structural `WorldInstance`, `InstanceMeasurements`, comparison assessment, capture, and attempt control. Inspect U1 source rather than recreating its types. Verify branch, status, and `NO_COLOR=1 didrun verify --strict` before edits.
...
- Use absolute argv execution with no shell and a sparse allowlist environment. Evidence allocation must consume the opaque `CandidateExecutionBinding` plus the actual matching `WorldPlan`; a candidate key or copied digests are insufficient. `WorldInstance` records only structural plan/candidate/stimulus/attempt/purpose/nonce/ordinal identity. Allocated roots, ports, PIDs, clocks, platform/tool facts, lifecycle, and cleanup belong in `InstanceMeasurements` or process/materialization receipts supplied by U2 edges.
...
5. Implement fresh attempt allocation and the execution state machine `ALLOCATED -> MATERIALIZING -> STARTING -> READY -> PROBING -> CAPTURING -> TEARING_DOWN -> FINALIZED`, while U2 exercises only applicable lifecycle phases. Construct the structural `WorldInstance` through the binding-plus-plan authority and retain host observations in measurement/receipt artifacts. Illegal transitions and partial/cancelled paths retain evidence without becoming eligible.
