...
- **Status:** controlling model; runtime authority is receipted through P07B-B, P07B-C product authority through C4 and process/verification maintenance through C4J are sealed and strict-clean, and C4L is the active `SOURCE_FULL` governance bootstrap with no product authority.
- **Historical C3 checkpoint:** C3 exact-target publication is active describes the earlier unreceipted checkpoint; C3 and C4 are now sealed, while C4L is the current repository boundary.
