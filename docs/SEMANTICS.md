...
A `StableBatch` binds one candidate, plan, stimulus, comparison envelope, the exact ordered set of per-repetition admitted-comparison digests, stimulus-independent comparison basis, exact admission roster, attempt purpose, derived repeat-policy identity, caller-declared schedule ordinals, and tagged trials. Each trial names the admission matrix and row token used for that repetition. Within-batch attempt and world digests must be globally unique. U1 checks ordinal consistency but establishes neither physical sequential execution nor candidate rotation; schedule rotation is exactly `NOT_ESTABLISHED_IN_U1`. The wire fact `duplicate_evidence_within_batch:false` states only structural nonduplication inside that batch; it does not establish physical freshness.
