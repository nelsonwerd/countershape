...
- **Status:** controlling model; runtime authority is receipted through P07A, P07B-A1 source/child-bind Observation, P07B-A2.1 current-ruling compilation preparation, accepted P07B-A2.2 pure compiler output, and sealed P07B-B terminal publication/retryable native materialization source; P07B-C C0 authority and C1 inert canonical-semantics source are sealed and strict-clean, while tracked C1 grades require a separately sealed C1B receipt reconciliation
...
The P07B-B publication/materialization paragraphs describe sealed source commit `eb06bdcf18f8e14db1257e73733afdc05cac045e`; only the exact commands mapped in its status document inherit their verbatim grades. The P07B-C paragraphs below retain sealed C0 authority and are projected by the sealed, strict-clean C1 source at commit `2fceacecbacb89fd7650f1570b2af33e6ea25ed3`. C1 values remain inert: they can be constructed, parsed, canonically encoded, hashed, compared, and classified from already-held values, but they confer no store, Git, materialization, runtime, process, publication, spawn, or study-head authority. Tracked C1 grades are authoritative only through an agreeing source receipt declaration, Git note, status, and handoff plus a separately sealed C1B boundary.

`ContractExecutionTarget` is a separate immutable nonhead pre-spawn authority. It binds one reopened bundle/residue, one explicit Git-pinned inspected and verified private materialization, one fresh durably allocated conformance attempt, and one measured/revalidated explicit Node executable plus owned runtime probe. C1 records checkpointed path/content identity: the admitted absolute path, executable-byte digest, executable mode/count, measured `process.execPath`, and runtime tuple. Those facts do not bind the filesystem object that a later spawn actually executes and do not close a coordinated replacement race; C3/C4 must revalidate at their physical boundaries and retain that residual risk. The target is official only when the store reopens both its canonical bytes and a typed store-private publication witness; generic CAS object existence, a parsed body, copied OIDs/digests, or copied runtime strings remain inert. It does not reuse `WorldInstance`, whose authority remains closed over the historical 2–4-candidate comparison plan. Dirty working-tree bytes are never represented or executed.

Before spawn, one store-private `ExecutionInterlock` serializes subject admission for the measured Darwin boot session. It is an operational CAS, never canonical evidence, result discovery, or a process-status head. Only the C4 contract runner may acquire it from an opaque C3-issued official-target capability. The runner then creates/reopens the target-keyed `StartClaim`; only that combined interlock/claim winner receives one process-local `RunPermit`. Generic publish/read, a C2 test fixture, losing caller, copied owner data, or restart can never mint or reissue it. A distinct `SpawnObservation` records `START_ERROR` or an observed child PID; missing observation is unknown and cannot enter an FCR. The exact claim ceiling is serialized cooperative at-most-once spawn admission per target, not exactly-once execution, child absence after crash, or safe lease takeover. Conclusive owner closure makes the terminal witness durable before releasing the interlock. Any ambiguity consumes the target and leaves the interlock held on that boot session; a fresh target alone is insufficient. Reset requires explicit operator action plus a measured Darwin boot-session identity demonstrably distinct from the held identity; otherwise P07B-C refuses every further subject spawn.

`FinalizedContractRun` is a separate immutable nonhead physical-run authority. It references one exact official `ContractExecutionTarget`, its exact attempt, intent claim, and owner-produced terminal facts. One bounded canonical `ClosedRunWitness` carries typed summaries/refs; raw or large captures stay private behind explicit count/byte ceilings, retention-at-finalization metadata, and default omission. Private placement is not a canonical confidentiality field or a confidentiality result: evidence may contain secrets, and confidentiality is not established. One process-control owner preserves at most one primary cause plus independent `TEARDOWN_ERROR` and `ORPHAN_RISK` controls, so cleanup cannot overwrite the original cause. Standalone scope is a separate closed `COMPLETE | PARTIAL | VIOLATED` result over target inventory, child bindings, import resolution, service bindings, and named parent-secret sentinel inheritance. Only a clean lifecycle with no primary or cleanup control, exact `PROJECTED` observation, and standalone `COMPLETE` without violation yields `ELIGIBLE_CLEAN`; partial/violated scope is structurally ineligible and is not relabeled as a process `ControlReason`. Publication waits for materialization/runtime revalidation, process completion, teardown, drains, orphan inspection, scope closure, and finalization. A target or attempt mismatch refuses publication/classification.
