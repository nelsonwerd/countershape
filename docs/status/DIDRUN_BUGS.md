...
- **Observed under long real verification:** U4 mutation event `415` ran for 201.95 seconds and repeated-study event `416` ran for 332.42 seconds. Neither surfaced bounded child progress while running; only the durable wrapper summary appeared after completion. This is safe for evidence retention but poor for a human supervising a long agent session, because silence is indistinguishable from a stuck verifier until the final event arrives.
