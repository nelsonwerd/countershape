...
17. Teardown probes the originally measured group immediately before `TERM`, skips the signal on observed absence, retains uncertainty on probe error, then waits a bounded grace interval, sends `KILL` when necessary, waits for the direct child, completes bounded pipe drains, and performs a final group-existence probe. Darwin's transient positive-presence `EPERM` state during grace is retried; if it persists at the deadline it remains uncertainty and cannot authorize blind KILL. A successful direct-child wait is not by itself cleanup authority. Only nil wait error or ordinary `ExitError` completes the direct wait edge; any unexpected wait failure, incomplete drain, or final-probe uncertainty conservatively sets orphan risk even if the original PGID is later absent.
