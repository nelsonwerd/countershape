...
- **Status:** controlling design; U1–U6 and P07A receipts are enumerated without reinterpretation in their status files; P07B-A1 source reconstruction and portable child-bind Observation are sealed in `status/P07B-A1-SOURCE.md`; the current-ruling compilation join, compiler, residue, materializer, target, execution, and product surfaces remain future
...
P07B-A2 must separately open and revalidate the current ruling/Choicepoint/FreshConfirmation, require the source execution binding to equal every retained confirmation binding, independently retranslate reopened projection proofs under the source-matched profile, and revalidate the selected-tuple partition before compiler bytes exist. Source retains no proof or ruling tuple bytes. A current ruling alone cannot recover raw fixture, seed, stdin, or body bytes retained historically only by digest.

The historical inherited-listener HTTP profile remains nonemittable. A1 exercised the new child-bind profile through a physical Observation, but the fresh confirmation, portable Choicepoint, selected-field ruling, and compilation join under that lineage remain A2 prerequisites. The future generated HTTP harness uses raw `node:net` bytes and the exact Go HTTP/1.1 grammar; high-level normalizing HTTP calls are outside the profile.
...
| U6c / P07B-A2/B/C | current-ruling compilation authority, recoverable compiler and Go/Node corpus, bundle publication, retryable materialization, target-inventory absence, and immutable nonhead current execution | remains future until its independent source/ruling, parity, stale-publication, recovery, runtime, and absence gates pass |
