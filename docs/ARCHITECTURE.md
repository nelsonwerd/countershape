...
- **Status:** controlling design; P07B-C through C4P is sealed and strict-clean; C4 CLI-profile physical execution and classification are active and `UNRECEIPTED`, while HTTP completion and product surfaces remain future
- **Historical C3 checkpoint:** P07B-C through C3S is sealed and strict-clean; C3 exact-target publication is active. That sentence names the pre-C3-seal checkpoint only; the current boundary is C4 as stated above.
...
### Active C4 CLI execution boundary
...
| U6c / P07B-C | C0 authority plus sealed C1/C2/C3/C4V/C4M/C4N/C4P prerequisites and active C4 CLI-profile physical execution, five-domain CLI scope closure, finalized-run durability, and classification publication | C1/C2/C3P grades enter tracked authority only through their separate receipt descendants; C4 is `UNRECEIPTED` until its exact 80-claim seal; HTTP/two-profile completion remains C5, interlock state has no result authority, and study head remains terminal residue |
