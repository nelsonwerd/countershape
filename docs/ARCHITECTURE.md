...
- **Status:** controlling design; U1–U6, P07A, every P07B-A/B boundary, both P07B-C C0 boundaries, and C1 inert target/run/classification source commit `2fceacecbacb89fd7650f1570b2af33e6ea25ed3` are sealed and strict-clean; tracked C1 grades require a separately sealed C1B receipt reconciliation, while store, physical execution, and product surfaces remain future
...
`DecisionRecord`, `ContractBundle`, `ContractExecutionTarget`, `FinalizedContractRun`, and `ContractExecution` are separate truth objects. Bundle hashing proves byte integrity, not authorship or conformance. The store revalidates the complete expected `RULING` token before any object/temp creation, publishes and reopens the bundle before advancing to terminal `RESIDUE`, and materializes only afterward from recoverable bytes. Materialization failure after residue is retryable and does not imply rollback. A later target is published and reopened before spawn from one explicit pinned Git materialization, the reopened bundle/residue, a fresh conformance attempt, a measured Darwin boot session, and an admitted Node runtime; it is not a `WorldInstance` and never advances the study head. The target's runtime tuple is checkpointed path/content identity, not proof that a later spawn consumed the same filesystem object. C3/C4 must revalidate at the physical boundary and still disclose coordinated same-user substitution as residual risk. Generic CAS bytes do not make the target official: typed store-private witnesses and exact-key create-once relationships reopen the capability. One store-private `ExecutionInterlock` is the sole mutable operational exception to the nonhead model. The C4 runner acquires it from the C3-issued official target, then creates StartClaim and receives one process-local permit; intent/interlock state is never proof that a child started. Missing spawn observation leaves the interlock held on that boot session, so a fresh target alone cannot overlap a possible survivor. Only a conclusive durable terminal witness releases it; ambiguous reset requires explicit operator action plus a measured boot-session identity demonstrably distinct from the held identity, or no further subject spawn is admitted. After the physical attempt closes, `FinalizedContractRun` binds that exact target/attempt to a bounded closed-run witness with separate process-control and standalone-scope axes. Only clean projected process closure plus complete unviolated scope is eligible. `ContractExecution` then binds the exact target/run plus a classifier profile and contains only the derived result class; it independently authors neither tuple nor reason. Future C4 publication persists that body. Classification-only retry never reruns a process.

C1's entire production topology is `internal/contractexec/model`. It imports only canonical/domain/value and sealed bundle-model semantics; it has no store, Git, world, runtime, process, network, CLI, server, randomness, clock, or publication edge, and no production package imports it before C2. Its C1 unit scope enumerates all nine production plus five test files individually with no prefix allowance; the architecture gate separately pins production/test filenames, production/test imports, and all 22 top-level test/fuzz symbols. The three schemas are closed syntax projections, while the Go package is semantic authority for cross-field correlations and derived results. `ClassifierProfileDigest()` derives the future C2 relationship key from the canonical schema/kind/literal-profile declaration; the digest is not serialized in `ContractExecution` and commits no implementation bytes. C1 accepts the closed start-error wire structure only. A later physical owner must establish evidence content and chronology. The inherited P07B-B architecture checker remains a current cumulative gate: every sealed B digest/import/API/publication/materialization invariant still runs, its former total-absence C premise admits names only beneath the exact inert package prefix after the complete C1 architecture gate succeeds, and foreign C surfaces still fail. Composition is one-way (`B -> C1`); the C1 checker must never invoke B, preventing recursive phase gates and keeping C1 the single authority for its exact model topology.
...
| U6c / P07B-C | C0 authority plus C1 inert canonical target/run/classification algebra; later typed nonhead publication, intent-only at-most-once spawn admission, bounded finalized run, five-domain scope closure, and immutable classifier-profile-bound execution | C0 authority/receipts and C1 source are sealed and strict-clean; tracked C1 grades require a separately sealed C1B receipt reconciliation, while store/runtime/process authority remains future until C2–C6 independently seal |
...
- releases an ambiguous interlock or admits another subject on the same boot without a conclusive terminal witness;
