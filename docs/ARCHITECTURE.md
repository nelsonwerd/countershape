...
- **Status:** controlling design; P07B-C product authority through sealed C4 and process/verification maintenance through sealed C4J are strict-clean. C4L is the active `SOURCE_FULL` governance bootstrap with no product authority; HTTP/two-profile completion remains C5 and is blocked until C4L seals.
- **Historical C3 checkpoint:** P07B-C through C3S is sealed and strict-clean; C3 exact-target publication is active. That sentence names the pre-C3-seal checkpoint only; the current repository boundary is C4L as stated above.
...
C3F B future-surface admission maintenance is sealed; its historical zero-member or complete four-pair admission remains the inherited B boundary for C3.
...
12. **Admission does not outrun freshness.** After the durable run owner is acquired, the C4 runner creates a detached bounded closure context and reopens the exact official target again in the contiguous guarded window immediately before permit consumption and `Start`. Drift leaves the target consumed and performs no spawn.
13. **Classification does not outrun durable release.** Publication reopens the exact StartClaim and revalidates the finalized-run release relationship immediately before persisting `ContractExecution`; a cached release bit, missing receipt, or corrupt relationship cannot classify.
14. **Private evidence rosters preserve multiplicity.** The target/attempt/StartClaim-bound manifest compares the sorted logical-reference sequence exactly. Duplicate references are a mismatch, not a set-equivalent roster.

## Truth jurisdictions

| Question | Sole authority | Bound by | Does not establish |
| --- | --- | --- | --- |
| Which source was selected? | `TreeIdentity` | object format plus immutable commit/tree OIDs | repository-handle identity, display spelling, Git version, materialized bytes, dependencies, or behavior |
...
| What physically finalized against that target? | immutable nonhead `FinalizedContractRun` | exact target/attempt/start intent, bounded closed-run witness, separate process and `COMPLETE | PARTIAL | VIOLATED` scope axes, exact projected tuple when present | conformance classification, another target/attempt, host-wide absence, containment, or confidentiality |
| How is that exact finalized run classified? | immutable nonhead `ContractExecution` | exact target digest, exact matching finalized-run digest, literal classifier profile, derived conformance or ineligible class | independent tuple/reason authorship, process rerun, study-head advancement, or overall correctness |
...
internal/contractexec/     exact-digest nonhead target, finalized-run, and classification services
internal/contractexec/runner/ sole C consumer of official target/admission authority and low-level mechanics
internal/processmechanics/ import-restricted Darwin mechanics; no store, semantic body, or RunPermit knowledge
...
2. Record repository fingerprint, Git version, and display spelling as local provenance around the content-portable object-format/commit/tree identity.
...
Materialization is offline. A missing promisor object is a refusal, never permission to fetch. `TreeIdentity` is content-portable across independently opened repositories that already contain the exact immutable objects; repository fingerprint, Git version, and display ref remain local provenance and live-revalidation facts rather than serialized selection identity. This is not clone resistance or permission to fetch. The portable tree digest excludes absolute root, current time, random attempt identity, and target-specific inode facts. Attempt allocation and host facts belong to U2 measurement/process artifacts, not the structural `WorldInstance` identity.
...
`DecisionRecord`, `ContractBundle`, `ContractExecutionTarget`, `FinalizedContractRun`, and `ContractExecution` are separate truth objects. Bundle hashing proves byte integrity, not authorship or conformance. The store revalidates the complete expected `RULING` token before any object/temp creation, publishes and reopens the bundle before advancing to terminal `RESIDUE`, and materializes only afterward from recoverable bytes. Materialization failure after residue is retryable and does not imply rollback. A later target is published and reopened before spawn from one explicit pinned Git materialization, the reopened bundle/residue, a fresh conformance attempt, a measured Darwin boot session, and an admitted Node runtime; it is not a `WorldInstance` and never advances the study head. The only v1 boot-session profile is `DARWIN_KERN_BOOTSESSIONUUID_V1`: the private host-epoch owner reads `kern.bootsessionuuid` twice through a bounded fixed-name Darwin system-call edge, validates each 36-character hyphenated nonzero UUID, lowercases each sample, compares the normalized values, and derives the target's typed `DarwinBootSessionIdentity` digest. It has no raw-value constructor and never falls back to `kern.boottime`, wall time, uptime, PID, a file, process-local state, `/usr/sbin/sysctl`, or PATH. Unsupported, malformed, or unstable measurement refuses authority. The profile establishes only what the OS currently reports, not physical reboot, child absence, clone resistance, VM non-rollback, kernel trust, or safe reset. `OfficialTarget.Valid()` is structural and evaluates sealed in-memory snapshots plus shared close/repository-revocation cells only; it performs no durable-store, filesystem, probe, or runtime freshness read. The returned `ContractTargetRecord.Valid()` remains a separate durable-store revalidation, and only `ReopenOfficialTarget` refreshes the complete physical graph. C4 must use a newly returned reopen immediately adjacent to its physical admission/spawn boundary. The target's runtime tuple is checkpointed path/content identity, not proof that a later spawn consumed the same filesystem object. The fixed owned probe establishes only the exact tuple reported by the explicitly selected cooperating executable; it does not prove vendor provenance or that a hostile executable could not emulate the Node response. C3/C4 must revalidate at the physical boundary and still disclose coordinated same-user substitution as residual risk. Generic CAS bytes plus C2's typed witness and create-once relationship produce only a durable inert storage record. Different valid targets raced for one attempt are first-durable-writer-wins, not deterministically arbitrated; the C3 issuer avoids that ambiguity by allocating one fresh attempt per publication. C3 alone may rejoin that record with live Git/materialization/runtime/attempt/boot authority and return `OfficialTarget`; C4 alone may turn a fresh interlock-plus-new-StartClaim outcome into its process-local permit. These package/topology boundaries constrain cooperating Countershape code and are not security boundaries against arbitrary same-user code. One store-private `ExecutionInterlock` is the sole mutable operational exception to the nonhead model. Target, caller, HELD state, and StartClaim boot identities must join exactly, while a historical `CLEAR` boot remains transition history rather than C3's live-boot authority. The C4 runner acquires the interlock from the C3-issued official target, remeasures the UUID profile, then creates StartClaim and receives one process-local permit; intent/interlock state is never proof that a child started. Missing spawn observation leaves the interlock held on that boot session, so a fresh target alone cannot overlap a possible survivor. Release requires the exact private manifest, finalized run, and target-to-run record to be durable and reopened first. The resulting `CLEAR` is admission-valid only after its exact clear receipt is durable and reopened; a bare or invalid `CLEAR` remains ambiguous and blocks admission. The receipt marks the trusted writer's operational transition and independently proves neither terminal causation nor child absence. Ambiguous reset requires explicit operator action plus a differently measured boot-session identity; even that difference is only an admission fence and does not prove the prior child absent. After the physical attempt closes, `FinalizedContractRun` binds that exact target/attempt to a bounded closed-run witness with separate process-control and standalone-scope axes. Only clean projected process closure plus complete unviolated scope is eligible. `ContractExecution` then binds the exact target/run plus a classifier profile and contains only the derived result class; it independently authors neither tuple nor reason. C4 publication persists that body. Classification-only retry never starts or reruns the contract subject and never consumes subject admission; the required live target reopen still executes two bounded owned Node runtime probe processes.

C1's entire production topology is `internal/contractexec/model`. It imports only canonical/domain/value and sealed bundle-model semantics; it has no store, Git, world, runtime, process, network, CLI, server, randomness, clock, or publication edge, and no production package imports it through C2. C2 validates already-held canonical model bytes through `SemanticObject` and typed joins without becoming semantic authority. Its C1 unit scope enumerates all nine production plus five test files individually with no prefix allowance; the architecture gate separately pins production/test filenames, production/test imports, and all 22 top-level test/fuzz symbols. The three schemas are closed syntax projections, while the Go package is semantic authority for cross-field correlations and derived results. `ClassifierProfileDigest()` derives C2's relationship key from the canonical schema/kind/literal-profile declaration; the digest is not serialized in `ContractExecution` and commits no implementation bytes. C1 accepts the closed start-error wire structure only. A later physical owner must establish evidence content and chronology. The inherited P07B-B architecture checker remains a current cumulative gate: every sealed B digest/import/API/publication/materialization invariant still runs, and its former total-absence C premise admits the inert C1 package, three exact C2 nonhead object-kind pairs, and the exact C2 classifier-profile support-kind pair. C3A cumulative-admission maintenance separately admitted the exact type pair `internal/contractexec/target.go:ContractExecutionTarget`. C3F conditionally admits the future C3 composition surface under an all-or-none rule as one complete exact four-pair bundle: `internal/contractexec/target.go:{ContractExecutionTarget,NewContractExecutionTarget,ParseContractExecutionTarget}` plus `internal/store/nonhead_contract.go:ParseContractExecutionTarget`. Historical zero-member state and the complete bundle are admitted; every partial bundle, extra family token, renamed path, sibling package, and reserved-root prefix/suffix variant—including the tested Unicode suffix—remains foreign. The conservative scanner inventories word-shaped raw-source tokens in non-test Go files under `internal/`, including comments and literals; it is an internal production-Go policy ceiling, not semantic call proof, a repository-wide scan, runtime-string detector, or general Unicode-confusable detector. C3's compiler/architecture and runtime gates must prove actual ownership and behavior. C3A separately aligned the canonical model with the schema/live-probe 128-byte runtime-version bound. Composition remains acyclic: C3 invokes B, B invokes default C1, and C3 must evolve default C1's topology mode within its already-frozen checker paths before target issuance can pass.

One normative corpus must exercise Go and Node strict parsing, field selection, missing/empty distinctions, CLI and raw HTTP capture profiles, projection bytes, and eligible/ineligible errors. Mutations that accept duplicate names, use independent allow-many value sets, assert a context-only field, normalize HTTP wire differences, or collapse ineligibility into contradiction must fail in both authorities. Standalone remains `UNRECEIPTED` until this corpus and observer-produced `COMPLETE` evidence for target inventory, child bindings, import resolution, service bindings, and named parent-secret sentinel inheritance pass on the exact Node path/version/major, native OS, and architecture named in the receipt. A runtime-generic bundle profile alone receipts none of those tuples.

The C3 sole-issuer AST policy builds import-qualified, module-wide named-type and function graphs from the Go build selection for the admitted Darwin/arm64/cgo profile. It preserves source-ordered lexical block/function/receiver type-parameter scopes and structurally substitutes instantiated generic arguments through output-bearing positions, including union constraints and dependent, renamed, or swapped receiver constraints. Known module generic function calls and function values are checked by result position; direct and shallowest-promoted generic receiver methods are included for calls, method values, and method expressions; and parenthesized callee and built-in forms are normalized before resolution. Omitted inferred output type parameters and unresolved external generic calls with target-bearing type arguments fail closed, while input-only generic consumers remain admitted. It rejects direct and recursively wrapped output types, constructed return values, function-local target-bearing wrapper declarations, exported target-bearing variables and selected/indexed fields (including the admitted inferred initializer forms), non-holder composites, and struct/interface-held output factories across the current `internal/` production tree. Indexed module function/method call multi-results are tuple-positioned, and embedded-field lookup follows shallowest-depth promotion with conservative equal-depth ambiguity. Its exact exported declaration/field inventory includes anonymous embedded fields, so a promoted surface cannot appear without changing that inventory. It is a static cooperating-code shape check, not complete dataflow analysis; local value/function shadowing and non-call comma-ok tuple projection are not modeled exactly, and arbitrary local identifier/call dataflow through `any`, reflection, generated code, and arbitrary same-user code remain outside it, while private state/seal ownership and frozen package topology remain the minting boundary.

### Sealed C4 CLI execution boundary

C4 implements the first physical contract-execution slice for the CLI profile. `internal/contractexec/runner` is the sole production caller of `AcquireContractRunOwner`; the architecture checker inventories the store definition and that one runner reference across production Go code. This is a source/topology property for cooperating code, not complete dataflow analysis or a boundary against arbitrary same-user programs.

After owner acquisition, the runner derives a detached, bounded terminal-closure context and performs a fourth full `ReopenOfficialTarget` in one exact guarded six-statement window: reopen, terminal error guard, exact-freshness guard, target replacement, deferred close, then `consumeAndStart`. The permit is consumed immediately before `Prepared.Start`. Preparation retains one inert defensive identity from the first fresh target: canonical target model, bundle, seven physical roots, candidate root, and marker path. Each later full reopen validates its retained predecessor under that predecessor's read lock, reconstructs the complete physical graph, and then compares the newly returned model and roots with that frozen identity. Reusing those inert values after the comparison removes redundant deep `OfficialTarget` getter validation; it skips no reopen, runtime probe, Git/materialization check, boot measurement, or spawn-adjacent edge. The real fixture closes both classification branches: the historical enrolled reference remains `CONTRADICTS`, while a second real ruling compiled over the same enrolled two-file reference tree produces an exact two-field `CONFORMS` tuple (`cli.stdout.json.mode=argv`, `cli.stdout.json.source=argv`). Ineligible controls remain separate.

The repeated physical fixture has an independent test-authority topology. Reference, conforming, forbidden, and ordinary hostile-control roles own distinct store pointers and canonical roots; forbidden and control compile exact reference-bundle clones, while conforming must retain a different bundle digest. The target-mutation control uses a fresh one-shot store because its correct post-admission refusal leaves that namespace held. The three qualification workflows run behind both Go's `-parallel=2` ceiling and a capacity-two package semaphore. This permits bounded pairing inside one repetition only; neither Go nor the verifier permits separate count iterations to overlap. These fixture mechanics make repeated proof practical without changing production admission, freshness, closure, or classification semantics.

Classification publication reopens the exact StartClaim and revalidates the durable finalized-run release relationship after classification readiness and immediately before persistence. Deleting or corrupting that relationship blocks publication even when an in-memory release bit exists. Private manifests retain the full sorted logical-reference sequence and therefore reject duplicate references; they remain bounded to 16 blobs and 64 MiB. Child invocation evidence is a canonical candidate-written receipt bounded to 64 KiB and joined to a fresh attempt ID plus exact logical argv before retirement. It is digest-backed cooperating-fixture evidence, not hostile-process attestation.

Private-evidence capacity is admitted during preparation, before `AcquireContractRunOwner`. The closed CLI profile permits at most 16 MiB per output channel and conservatively reserves one 1 MiB capture-frame overhead, both channel limits, one 3 MiB projection frame, and twelve 1 MiB canonical summaries: 48 MiB of unique private bodies beneath the store's 64 MiB ceiling. Stdout and stderr are persisted once in one raw length-framed body; the logical `DRAIN_RESULT` and `CAPTURED_OBSERVATION` references intentionally resolve to that same digest. Projection bytes and their exact tuple use a second raw frame. Immediately before private-manifest persistence, the runner recomputes the actual unique-body aggregate, caps every draft at 15 logical kinds and 14 unique blobs, and refuses any escape from the pre-owner envelope. The full projected child draft reaches both caps; controlled or start-error drafts may be smaller.

Post-child transient closure never recursively enumerates or removes candidate-writable roots. Evidence, scope, short-socket, attempt, and attempt-evidence rosters use retained directory identity plus descriptor reads capped at the exact expected cardinality plus one, with no-progress and foreign-name refusal. Exact scope cleanup removes only the retained socket, module, and alias leaves and then the proven-empty directories. A foreign, nested, missing, or replaced scope leaf makes the measurement ambiguous and is left in place for outer attempt cleanup; foreign invocation evidence hard-refuses finalization and is likewise not traversed. This is bounded cooperative cleanup, not secure deletion or protection from a same-UID actor racing the final identity-check-to-remove edge.

Candidate inventory measurement is independently resource-bounded before spawn and again after terminal closure. C4 opens each directory and regular file with Darwin's all-component no-follow flag, walks directory descriptors in batches of 128, streams exactly the admitted file size into SHA-256 without retaining whole bodies, and refuses beyond 20,000 entries, 32 MiB of cumulative relative-path bytes, depth 128, 64 MiB for one file, or 64 MiB aggregate file content. Directory and file path/descriptor identities are compared before and after each bounded read. These are intentionally narrower C4 CLI-profile ceilings than the generic materializer's hard admission envelope; exceeding them refuses execution/evidence closure. They bound work and persistent drift but do not prevent a same-UID actor from swapping and restoring an object between observations.

This boundary is deliberately CLI-only. Clean target inventory is earned only by the exact enrolled cooperating two-file fixture; the runtime probe is a cooperating-executable tuple rather than Node-vendor authenticity; named sentinel evidence proves only omission of the named environment keys; and coordinated same-UID replacement/TOCTOU remains outside the guarantee. C4 proves no host-wide absence, network denial, listener ownership, containment, confidentiality, secure deletion, survivor absence, exactly-once execution, or process resume. HTTP and full two-profile completion remain C5 work.
...
Object edges are digest references inside immutable bodies. A new revision never edits an ancestor. U6a's public typed spine advances through `SOURCE_PLAN -> BASELINE -> DIVERGENCE -> REDUCTION -> CONFIRMATION -> CHOICEPOINT_READY -> RULING`. The internal stage table reserves the terminal `RESIDUE` slot; P07A does not expose it, and P07B owns construction-safe `ContractBundle` publication. ContractExecutionTarget, FinalizedContractRun, and ContractExecution are nonhead objects, never stages. `PARTIAL` and `CANCELLED` do not advance the head. Only the current head plus immediate predecessor object/head digests are retained in `head.json`; current U6 canonical objects and heads have no archived head-body history, durable staleness/invalidation record, branching, deletion, garbage collection, retention, or administrative recovery. C2 may explicitly purge only non-authoritative private evidence packs. A durable no-serve tombstone precedes best-effort cleanup; `PURGED` is logical unavailability, not secure erasure. Purge or unexpected loss never deletes or mutates canonical target/run/classification objects, relationships, heads, or the finalized-retention fact, and current availability is reported separately.

The receiptable U6/C2 storage authority is deliberately narrow: conforming Countershape writers on the natively exercised local Darwin filesystem. Darwin `flock`, same-directory hard-link create-if-absent publication, and atomic head/interlock rename protect cooperating writers. Direct same-UID mutation remains possible despite `0600` object modes. NFS/network/synchronizing filesystems, hostile same-UID races, forced power-loss recovery, and crash consistency beyond the exercised sync-and-reopen checks are not established. C2 adds `contract-execution/{links,operations}` plus `private-captures/contract-runs/{manifests,packs,purge-intents}` as private operational namespaces with retained directory identities. Relevant relation, publication, StartClaim, manifest, pack, purge, and fixed-root identities are reasserted at later authority and mutation boundaries; deterministic pre-call substitution fails closed, while replacement racing between validation and a leaf operation remains part of the same-UID nonclaim. These namespaces contain typed relationships, StartClaims, interlock/clear-receipt state, and private availability mechanics; they are not public traversal, current/latest status, execution journals, semantic heads, confidentiality controls, or secure-erasure mechanisms.
...
## Verification-profile and operator-transition boundaries

A unit's `verification_profile` governs the proof and claim surface of that repository boundary; it is not Countershape product authority. `SOURCE_FULL` retains the complete current cumulative battery. `RECEIPT_RECONCILIATION` remains the exact receipt-only profile. `NON_PRODUCT_MAINTENANCE` is dormant and prospective in v25: no current row consumes it, and C4L remains `SOURCE_FULL`. V25 installs grammar and partial scope primitives only; a later `SOURCE_FULL` authority migration must add one concrete row plus exact runbook, preseal, argv, and end-to-end fixture authority before use. A future narrow consumer is legal only when its sealed direct parent, itself closed through `SOURCE_FULL`, already declares the exact child, parent, empty-prefix path roster, ordered nine-claim manifest, receipt states, and profile fields. The candidate cannot add, widen, or reinterpret its own authority.

The narrow path ceiling is limited to parent-predeclared coordination/status/prompt/research documentation and boundary-specific maintenance checker/self-test tools. It excludes verifier/runtime implementation, product/runtime source, generated product, product schema/example and dependency authority, qualification catalogs, the core architecture/claim/semantic/state/threat/verification contracts, `spec/verification/p07b-c-unit-paths.json`, and both generic phase/profile checkers. `product_authority: "NONE"` and `product_behavior: "INHERITED_UNREPROVEN"` deny product and inherited outcome authority. `product_projection: "PARENT_FROZEN"` is a prospective fail-closed eligibility token, not a current receipt: before a future child can use it, that child's final gate must reopen the exact sealed parent and prove no tracked change outside the parent-predeclared roster relative to that parent. It carries no verifier outcome, didrun event or grade, product-correctness result, behavior proof, or presently enumerated product-projection digest.

`OWNER_OUT_OF_BAND` is transition-topology authority only. A `CITED_UNAUTHENTICATED` provenance may name one literal canonical `OWNER_INSTRUCTION_ARTIFACT` path and raw-byte SHA-256 only when those bytes already exist as exactly one nonempty, at-most-64-KiB valid-UTF-8 mode-`100644` regular blob in the direct parent's tree. Otherwise the exact state is `UNEVIDENCED`. Citation never authenticates owner identity or authorization; `authentication` remains `NOT_ESTABLISHED`, signed authorization remains `NOT_IMPLEMENTED`, and a same-unit companion file cannot upgrade either ceiling. The v25 rows for C4H, C4I, C4K, C4J, and C4L are all `UNEVIDENCED`.
...
| U6c / P07B-B | opaque node-issued terminal publication, complete predecessor restart, and retryable exact native six-file materialization | complete boundary sealed and strict-clean through final handoff `464e47adbf7f4497dfafa89938a9539239ffd41b`; no target-inventory, runtime, run, classification, or product claim |
| U6c / P07B-C | C0 authority plus sealed C1/C2/C3/C4V/C4M/C4N/C4P prerequisites, sealed C4 CLI-profile physical execution, and sealed C4H/C4I/C4K/C4J process-verification maintenance; active C4L is a `SOURCE_FULL` governance bootstrap with no product authority | C1/C2/C3P grades enter tracked authority only through their separate receipt descendants; every C4L grade remains `UNRECEIPTED`; HTTP/two-profile completion remains C5 and is blocked until C4L seals, interlock state has no result authority, and study head remains terminal residue |
...
- spawns before the exact official ContractExecutionTarget is durably published/reopened and the private interlock plus StartClaim are durably acquired;
- treats a StartClaim, lease, or lock as proof that a child started, stopped, or is absent;
- mints/reissues a RunPermit from generic publish/read, an existing claim, copied owner data, or restart;
- lets low-level process mechanics accept a RunPermit/target digest, or lets any package other than the contract runner consume C admission authority;
- releases an ambiguous interlock, releases before the exact FCR and target-to-run record reopen, or admits another subject on the same boot without durable terminal closure;
- treats receiptless or invalid `CLEAR` as released, uses a clear receipt as child-absence evidence, or treats its historical boot digest as live C3 authority;
- pairs a FinalizedContractRun with any target or attempt other than the exact one it names;
- treats PARTIAL or VIOLATED standalone scope as eligible or as a process ControlReason;
- lets ContractExecution duplicate target/runtime/lifecycle/observation authority or become a study-head transition;
- creates a semantic execution list, mutable result/status record, filesystem traversal, or latest/current selector; the private interlock may only block spawn and carries no result/discovery authority;
