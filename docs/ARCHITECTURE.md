...
- **Status:** controlling design; U1–U6, P07A, every P07B-A/B boundary, and both P07B-C C0 boundaries are sealed and strict-clean; C1 inert target/run/classification semantics are the current `UNRECEIPTED` source boundary, while their store, physical execution, and product surfaces remain future
...
| U6c / P07B-C | C0 authority plus C1 inert canonical target/run/classification algebra; later typed nonhead publication, intent-only at-most-once spawn admission, bounded finalized run, five-domain scope closure, and immutable classifier-profile-bound execution | C0 authority/receipts are sealed; C1 can claim only its exact inert semantic boundary after its own strict gate, while store/runtime/process authority remains future until C2–C6 independently seal |
