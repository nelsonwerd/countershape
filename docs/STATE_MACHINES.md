...
C6B_SEALED
  ...
  -> U7B_SEALED
  -> U7N_SOURCE_CANDIDATE
  -> U7N_SEALED
  -> U7C_SOURCE_CANDIDATE
  -> U7C_SEALED
  -> U7D_SOURCE_CANDIDATE
  -> U7D_SEALED
  -> U7Q_SOURCE_CANDIDATE
  -> U7Q_SEALED
  -> U7S_SOURCE_CANDIDATE
  -> U7S_SEALED
...
- `U7B_SEALED -> U7N_SOURCE_CANDIDATE` is a second owner-selected control amendment after the exact enrolled CLI stimulus and sealed U7B HTTP authority were found incompatible with the frozen future claims. It is `OWNER_OUT_OF_BAND`, `OWNER_AUTHORIZED_CLI_AND_MILESTONE_AUTHORITY_RECONCILIATION`, `UNEVIDENCED`, unauthenticated, unsigned, product-authority `NONE`, product-behavior `INHERITED_UNREPROVEN`, and grade-transfer `NONE`. It may change only its exact 16 control/documentation paths and cannot reinterpret U7B evidence.
- `U7D_SEALED -> U7Q_SOURCE_CANDIDATE` is a third owner-selected zero-product control amendment after the frozen U7R rules proved mutually unsatisfiable and an external Homebrew upgrade invalidated the pinned recorder launcher during the first failed U7Q attempt: one gate required a U7R/PRESENT HANDOFF capsule while another allowed only a pending-receipt-block replacement relative to the sealed parent. It is `OWNER_OUT_OF_BAND`, `OWNER_AUTHORIZED_RECEIPT_TRANSITION_AND_RECORDER_RUNTIME_EPOCH_CONTROL_REPAIR`, `UNEVIDENCED`, unauthenticated, unsigned, product-authority `NONE`, product-behavior `INHERITED_UNREPROVEN`, and grade-transfer `NONE`. It may change only its exact 15 control/documentation paths, binds the new active recorder epoch separately from sealed U7D's preserved source-study epoch, cannot reinterpret U7D evidence, and leaves the U7D receipt absent.
- `U7Q_SEALED -> U7S_SOURCE_CANDIDATE` is a fourth owner-selected zero-product control amendment after the first uncommitted U7R receipt projection proved that both source validators omitted the sealed harness's explicit subject `PATH` override while reconstructing process-environment digests. It is `OWNER_OUT_OF_BAND`, `OWNER_AUTHORIZED_SOURCE_STUDY_ENVIRONMENT_AUTHORITY_REPAIR`, `UNEVIDENCED`, unauthenticated, unsigned, product-authority `NONE`, product-behavior `INHERITED_UNREPROVEN`, and grade-transfer `NONE`. It may change only its exact 15 control/documentation paths, binds `PATH=/usr/bin:/bin:/opt/homebrew/bin` as source-harness authority, changes no study/evidence byte, and leaves the U7D receipt absent.
...
- `U7N_SEALED -> U7C_SOURCE_CANDIDATE` admits the independent CLI decision study plus one exact enrolled official reference execution. The enrolled reference stimulus selects normal exit `0`; the fixture's separate exit-`7` behavior is substrate coverage only. A clean signal is an eligible completion with a missing selected exit code and therefore contradicts the normal-exit tuple; an invalid overlay is a pre-spawn refusal with no execution; runtime escaped-write behavior and official-execution generalization remain unproven. U7C must complete its own three-run process/artifact gate before sealing and may not route CLI truth through HTTP.
- `U7C_SEALED -> U7D_SOURCE_CANDIDATE` admits only the exact product-owned cross-domain workflow and U7P-origin/U7N-amended harness-v2 process/artifact closure surfaces. The terminal milestone is bounded to both domains' process/artifact success, one exact CLI official execution, and HTTP direct-process observation; HTTP finalized-run/classification authority remains absent. It does not reopen frozen study ownership or turn the harness into an independent semantic oracle.
- `U7D_SEALED -> U7Q_SOURCE_CANDIDATE` admits no product or receipt authority; U7Q exists only to repair and independently test the future receipt-transition controls and active recorder-runtime epoch while preserving the sealed U7D source epoch.
- `U7Q_SEALED -> U7S_SOURCE_CANDIDATE` admits no product or receipt authority; U7S exists only to align the future source-study environment reconstruction with the sealed harness's explicit subject `PATH`, while preserving sealed U7D and U7Q.
- `U7S_SEALED -> U7R_RECEIPT_CANDIDATE` requires U7S to be the exact direct parent, sealed U7Q to be U7S's exact parent, and sealed U7D to be U7Q's exact parent and receipt source. The U7D receipt remains `ABSENT` through both sealed control edges.
- `U7R_RECEIPT_CANDIDATE -> U7R_RECEIPT_RECONCILED` is an exact three-path projection. Relative to sealed U7S, HANDOFF must perform exactly two canonical substitutions—the U7S-to-U7R phase capsule and the pending-to-receipt block—while the sealed-U7D status may replace only its pending block. It may bind only sealed U7D source facts and explicit nonclaims; it may not change product, checker, verifier, or study evidence and cannot receipt U7R itself.

Receipt state is monotonic inside this namespace: `ABSENT` for U7P/U7M/U7A/U7B/U7N/U7C/U7D/U7Q/U7S and `PRESENT` only after U7R. The inherited C3P, C3, and C6A receipts remain `PRESENT` throughout, but they confer no U7 behavior grade.
...
| `CONTRACT_EXECUTION_TARGET` | immutable nonhead `FINALIZED_CONTRACT_RUN` | the matching sealed CLI or active HTTP profile owner acquires the private boot-session interlock from opaque official-target authority, wins StartClaim/RunPermit once, records conclusive SpawnObservation, closes its physical axes and private manifest, then makes the exact FCR and target-to-run relationship durable and reopens both | no permit, missing observation, target/attempt mismatch, or incomplete terminal evidence refuses FCR publication; a later `CLEAR` or receipt failure cannot roll back an already durable FCR but leaves release incomplete and future admission blocked; study head remains the same bundle residue |
...
## P07B/U6c contract-emission state machine (A2.2 and B sealed)
...
The live boot checkpoint has one exact v1 profile: `DARWIN_KERN_BOOTSESSIONUUID_V1`. C3 reads the read-only Darwin `kern.bootsessionuuid` key twice through a bounded fixed-name system-call edge, validates each 36-character hyphenated nonzero UUID, lowercases each sample, compares the normalized values, and issues only an opaque capability over the typed `DarwinBootSessionIdentity` digest. A raw UUID/digest/timestamp cannot enter the transition. The retired clock-derived profile, `kern.boottime`, wall time, uptime, PID, filesystem or process-local fallbacks, a `sysctl` subprocess, PATH lookup, malformed data, sample disagreement, and unsupported platforms all refuse. The matching execution owner repeats the measurement immediately before admission. UUID equality or change is only an OS-reported epoch fence; it proves no physical reboot, prior-child absence, VM non-rollback, kernel trust, collision impossibility, or safe reset.
...
Before `CHILD_PID_OBSERVED`, preparation validates root metadata and declared seed topology, then acquires a nonblocking advisory lease on the exact no-follow attempt temporary-root descriptor before fixture-roster inspection or mutation. The holder admits only a compatible-partial exact fixture roster, creates one exclusive attempt-temporary staging root even for an empty seed roster, atomically publishes complete seeds, retires exact-empty staging, and proves the exact final roster while retaining the lease. Successful preparation transfers ownership across the complete execution attempt through receipt retirement, terminal persistence, and return. A live cooperating holder transitions the loser to `HTTP_CONTRACT_RUN_ADMISSION_REFUSED` before the loser can interpret the holder's visible receipt as residue; process death releases the lease. Without a live holder, pre-existing staging or receipt is crash/foreign residue and, like foreign fixture entries, special setuid/setgid/sticky modes, or identity drift, transitions to `HTTP_CONTRACT_RUN_FIXTURE_REJECTED` before spawn authority is acquired; no recursive repair transition exists.

After durable owner acquisition and before `CHILD_PID_OBSERVED`, the HTTP owner follows this bounded subchain:

```text
DETACHED_REVALIDATION_PHASE
  -> START_ADJACENT_FULL_TARGET_REOPEN
  -> DETACHED_CLOSURE_PHASE_CREATED
  -> REVALIDATION_DEADLINE_CHECKED_AND_CANCELED
  -> EXACT_TARGET_EQUALITY
  -> ONE_SHOT_START_AUTHORITY_CONSUMED
  -> PHYSICAL_START_ATTEMPTED
```

The closure phase is created before the revalidation phase is retired, so no scheduler gap is outside a deadline. Both use the unchanged source-derived phase budget. Reopen failure, revalidation expiry, equality drift, or closure expiry before one-shot consumption performs no successful spawn transition and leaves the durable StartClaim/interlock held. After consumption, closure expiry can follow a real start and cannot erase captured process/scope facts; held, ambiguous, or released state depends on durable progress. If release completed before classification persistence failed or expired, the interlock is clear and classification-only recovery remains available.

After `CHILD_PID_OBSERVED`, the active HTTP owner adds this exact subchain:

```text
EXACT_CHILD_READY_FRAME_AND_EOF
  -> ONE_RAW_LOOPBACK_REQUEST
  -> RAW_RESPONSE_CAPTURED_OR_TYPED_CONTROL
  -> PROCESS_GROUP_TERMINAL_CLOSURE
  -> FIVE_HTTP_SCOPE_DOMAINS_CLOSED
  -> INVOCATION_RECEIPT_RETIRED
  -> TARGET_INVENTORY_REVALIDATED
  -> PRIVATE_MANIFEST_AND_FCR_DURABLE
  -> RELEASE_RECONVERGED
  -> CONTRACT_EXECUTION_CLASSIFIED
```

A start error bypasses readiness and exchange but still persists a durable spawn observation, closes every measurable scope domain without erasing violations, produces an ineligible FCR, and releases only after exact terminal joins. Classification recovery has a narrower profile-bound nonspawn path:

```text
LIVE_TARGET_REOPENED_AND_HTTP_PROFILE_REVALIDATED
  -> DURABLE_TERMINAL_CLOSURE
  -> RELEASE_RECONVERGED
  -> SAME_CONTRACT_EXECUTION_REOPENED
  -> NO_SUBJECT_RESPAWN_AUTHORITY
```

The reported endpoint is consumed for one raw exchange but is never promoted to listener-ownership authority. A source-mutated decoy can reach clean process and projected-response states while remaining scope-ineligible. A close error on one of the six owned process-pipe endpoints adds one fixed phase diagnostic and sets teardown error without changing the primary process state or implying orphan risk; no retry transition exists. Every C5 transition remains `UNRECEIPTED` before the 79-claim seal.
...
| copied target/tree/source/runtime fields, parsed `ContractExecutionTarget`, fake one-member candidate set, or historical `WorldInstance` -> official target/run | target-authority refusal | only the C3 live issuer may return official-target authority, and only the matching exact CLI or HTTP profile owner may consume it |
| generic CAS read, C2 fixture, existing StartClaim, losing interlock/claimant, copied owner epoch, lease expiry, or restart -> RunPermit | `TARGET_START_ALREADY_CLAIMED` or interlock refusal; no permit | only the matching profile owner's combined interlock/claim durable winner may receive the one process-local permit |
