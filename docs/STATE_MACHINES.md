...
- `U7Q_SEALED -> U7R_RECEIPT_CANDIDATE` requires U7Q to be the exact direct parent and sealed U7D to be U7Q's exact parent and receipt source. The U7D receipt remains `ABSENT` through U7Q's sealed source edge.
- `U7R_RECEIPT_CANDIDATE -> U7R_RECEIPT_RECONCILED` is an exact three-path projection. Relative to sealed U7Q, HANDOFF must perform exactly two canonical substitutions—the U7Q-to-U7R phase capsule and the pending-to-receipt block—while the sealed-U7D status may replace only its pending block. It may bind only sealed U7D source facts and explicit nonclaims; it may not change product, checker, verifier, or study evidence and cannot receipt U7R itself.

Receipt state is monotonic inside this namespace: `ABSENT` for U7P/U7M/U7A/U7B/U7N/U7C/U7D/U7Q and `PRESENT` only after U7R. The inherited C3P, C3, and C6A receipts remain `PRESENT` throughout, but they confer no U7 behavior grade.
