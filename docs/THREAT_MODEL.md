...
- **Status:** design contract; P07B-C product authority through C4 and process/verification maintenance through C4L are sealed and strict-clean, C5 HTTP-profile physical execution/full-scope classification is active and `UNRECEIPTED`, and no security control is validated beyond its exact command/environment receipt
...
The frozen C3-era sentence “C3 exact-target publication is active” remains historical exact-tree context. C4 and maintenance through C4L are sealed, and the current C5 status immediately above governs.
...
| U6c / P07B-C | sealed CLI plus active `UNRECEIPTED` capability-only HTTP owner, normalized Darwin boot-session UUID, intent-only at-most-once spawn admission, exact readiness/raw exchange, bounded finalized run with separate five-domain scope, and classifier-profile-bound terminal recovery | retired/malformed/unstable/caller-authored boot profile or fallback, generic executable, copied/parsed target, dirty worktree, permit reissue, ambiguous-spawn reuse, runtime substitution, decoy-listener eligibility, metadata-shaped absence, forbidden dependency, contradiction/ineligibility collapse, private-evidence leak, or hidden execution head |
