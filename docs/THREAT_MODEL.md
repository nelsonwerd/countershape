...
Before later spawn, a future nonhead `ContractExecutionTarget` is constructed only from the reopened bundle/residue, one live Git-issued pinned/inspected/verified private materialization, a fresh durable conformance attempt, a measured Darwin boot session, and an admitted/revalidated explicit Node runtime; canonical bytes plus a typed store-private witness publish and reopen first. C2 storage mechanics cannot issue this authority. The C4 contract runner alone may consume the opaque C3-issued target, durably acquire a store-private boot-session `ExecutionInterlock`, create/reopen target-keyed StartClaim, and receive one nonreopenable process-local RunPermit. Low-level process mechanics never sees that permit or semantic body. Interlock/claim state proves no spawn, exit, cleanup, or survivor absence; a separate SpawnObservation records `START_ERROR` or observed child PID. Missing observation cannot enter an FCR and leaves the interlock held on that boot. A fresh target does not clear ambiguity; reset requires explicit operator action plus a measured boot-session identity demonstrably distinct from the held identity, or no further subject spawn is admitted. The five-domain standalone run measures target inventory, child bindings, import resolution, service bindings, and named parent-secret sentinel inheritance as COMPLETE, PARTIAL, or VIOLATED. It establishes no host-wide absence, network/registry denial, listener ownership, containment, or confidentiality. After terminal process and scope closure, a separate `FinalizedContractRun` binds the exact target/attempt to bounded typed summaries and private-evidence refs; the terminal witness is durable before the interlock releases. A separately persisted classifier-profile-bound `ContractExecution` then derives `CONFORMS`, `CONTRADICTS`, or `INELIGIBLE_EXECUTION` without accepting a tuple or process/scope reason, never reruns a process, and never advances the terminal study head. Direct generated Node TAP remains independent black-box/operator evidence.
