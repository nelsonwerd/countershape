...
- **Status:** design contract frozen at the C6A source-candidate epoch; P07B-C product authority through C5 and verifier-preflight maintenance through C5V are sealed and strict-clean, the C6A receipt is absent, and no security control is validated beyond its exact command/environment receipt
...
The frozen C3-era sentence “C3 exact-target publication is active” remains historical exact-tree context. C4/C5 and maintenance through C5V are sealed, and the C6A document epoch above governs this file; later operational state belongs to the handoff receipt-phase capsule.
...
C5 has no operator CLI or studio, so the quoted display is a future-surface acceptance criterion rather than a warning currently rendered by C5. C5 execution is maintainer-only through the controlled in-repository verification path, and it still runs the selected subject with the maintainer's user permissions and host network access.
...
**Required controls.** Publicly label the mode `HOST_ALLOWED`. Proof fixtures use no external hostname, TLS, redirect, proxy, cookie jar, compression, WebSocket, streaming body, or database. The historical inherited-listener fixture binds loopback and produces its fixture-owned readiness signal. C5's child-bind profile accepts exactly `COUNTERSHAPE_READY_V1 <port>\n` over inherited FD 3 followed by EOF, then uses the reported literal-loopback endpoint for exactly one raw request. Child/process, readiness, transport, raw response, projection, and service facts remain distinct. A source-mutated decoy may answer cleanly, but independent enrolled-inventory and five-domain scope closure prevent that observation from becoming eligible.
...
**Residual risk.** Candidate code retains normal host network access. Loopback does not prevent contact with other local services. `child_reported_port` proves the accepted child-process-tree frame and subsequent endpoint use, not that the reporting PID owns the listener; arbitrary trusted code can report a decoy local service. Countershape establishes neither listener authenticity, egress denial, proxy absence, nor readiness noninterference for arbitrary imported services.
...
C5 adds a distinct HTTP envelope of at most 15 bodies under the same 64 MiB store ceiling, exact raw readiness retention, and an attempt-bound seven-member invocation receipt. After owner acquisition it places the start-adjacent full target reopen in one detached bounded revalidation phase, creates the separately bounded closure phase before retiring the first, checks the revalidation deadline, and only then compares, consumes, and starts. Both phases keep the unchanged `readiness + probe + teardown + 30 seconds` budget. This prevents a load-delayed reopen from consuming closure authority, but increases the possible post-owner duration from one phase to roughly two and slightly enlarges the timer-only interval between reopen and consumption; no filesystem, subprocess, store, or caller-controlled operation is added there. Revalidation or pre-consumption closure expiry performs no spawn and leaves the claim/interlock held. Post-consumption expiry can follow a real child and yields progress-dependent held or ambiguous state; expiry after durable release can leave the interlock clear and classification recoverable. After root-metadata and declared-topology validation, but before fixture-roster inspection or mutation, a nonblocking advisory lease on the exact no-follow attempt temporary-root descriptor serializes cooperating execution attempts without a persistent lock artifact. A successful preparation transfers the lease through receipt retirement, terminal persistence, and return; a live holder therefore produces typed admission refusal before a loser can interpret the holder's receipt as residue, while process death releases the lease. The holder's descriptor-backed compatible-partial roster admits only declared directories and exact seed files. A fixed staging directory is created exclusively beneath the attempt temporary root even for an empty seed roster; synchronized staged inodes cross the same device through no-clobber hard links; staging must retire exact-empty and absent; and a final descriptor-backed roster proves exact `0700` directories, exact regular `0644` seed files, bytes, and identities. Without a live holder, pre-existing staging or receipt is crash/foreign residue, refuses before intended writes, and remains preserved. The lease coordinates conforming processes and is not a same-UID security boundary. The HTTP scope probe creates an attempt-private root plus a bounded short Unix-socket alias, rejects setuid/setgid/sticky bits at the root or on nested directories/files, validates exact descriptor rosters, detects same-size module mutation, and removes only retained same-identity leaves and proven-empty directories. Missing evidence and foreign residue remain ambiguous; neither is recursively traversed or rewritten as clean.

The six process-pipe endpoints close through one-shot pointer-owned wrappers. Once `Start` is attempted, errors become three fixed phase-level teardown diagnostics without replacing the primary result or creating orphan risk, and are never retried because the descriptor number may already have been reused. This proves only that `Close` returned an error for that phase, not that a kernel descriptor remained open; pre-Start cleanup, TCP, and other non-pipe cleanup are outside this claim. Recovery freshly reopens the target and validates the standalone HTTP profile before any terminal-store lookup. It may reconverge durable release and classification only and exposes no path to source preparation, StartClaim, RunPermit, or subject respawn, although live target validation may run bounded owned Node runtime probes. Exact sealed C5 receipts these bounded controls under its 79/79 `TREE-EXACT` ledger and strict exit `0`; those grades do not establish broader security properties and do not transfer to C6A.
...
## C6 evidence and review threats

| Threat | Failure mode | C6A control and residual |
| --- | --- | --- |
| Hand-entered summary authority | prose silently disagrees with sealed C5V or selected physical profiles | canonical summary/evidence derive from pinned parent artifacts and live exact profile transcripts; human interpretation remains outside the schema |
| Capture substitution or mixed generation | one stale JSON/text file is combined with a newer set | exact old/new manifests and identities including both sides of activation rename, exclusive fail-closed lock, four-state synced v3 journal, admission recheck, active validation, journal-derived quarantine cleanup resumable after partial deletion, idempotent pre-commit recovery with and without a prior generation, and committed-generation preservation; pre-journal or journal-temporary residue requires operator inspection, all unjournaled residue refuses, and hostile same-UID pathname races remain excluded |
| Path redirection | a symlinked ancestor routes evidence reads/writes outside the repository | component-by-component no-follow resolution and root confinement; this is a cooperating-process control, not filesystem sandboxing |
| Git authority substitution | replace refs or ambient Git configuration alter the reopened parent | fixed `/usr/bin/git`, `--no-replace-objects`, sterile Git environment, exact commit/tree/parent/subject/note identities; compromised Git/OS remains excluded |
| Incomplete execution/source authority | same-count test substitution, runtime-read data drift, or a synthetic cache file evades the admission digest | pure exact profile descriptors, Go present-source derivation, explicit runtime data, required `go.sum` absence, private-output exclusion, and byte descriptors enter admission; complete Go distribution/SDK/dynamic-library and host provenance remain unbound |
| Ambiguous nested Go-test ancestry | slash-delimited `Test` names are mistaken for explicit parent identifiers | exact top-level rosters and root-lifecycle enclosure are enforced, while per-name terminal/skip/output/pause rules remain closed; Go-JSON supplies no parent ID, so an intermediate parent cannot be distinguished from a prefix-colliding sibling and no stronger ancestry claim is made |
| Hidden terminal influence | control, bidi, non-ASCII, or over-width text changes how evidence appears | strict UTF-8 JSON, forbidden controls, ASCII-only rendered grammar, exact 60/80/120 ceilings; typography quality and accessibility remain human work |
| Self-receipt | C6A source claims its own future seal/note/grade | expert v4 closes early boundary/epoch/receipt/state, the terminal derives its banner from that object, the v1-compatible summary note repeats source-era `UNRECEIPTED` only as disclosure, and separate C6M source authority plus C6B receipt reconciliation remain mandatory |
| Local-artifact portability overclaim | ignored ledger/HTML paths are treated as portable or signed proof | exact local availability and digests are disclosed; HTML is a snapshot and the ledger is required for didrun strict reproduction |
| Critic-authority laundering | model opinion becomes correctness, comprehension, or security evidence | critic sees only sanitized captures, remains `UNRECEIPTED`, and cannot override deterministic gates or authorize a state transition |
| Sensitive review input | raw ledger blobs, private roots, environment, or credentials are sent to a model/API | critic input is limited to tracked sanitized captures and rubric; real external API calls remain human-gated and are never faked |

These controls make the evidence surface harder to spoof accidentally. They do not constitute an independent security review, hostile containment, a signed provenance system, or a confidentiality guarantee.
...
| U6c / P07B-C | sealed CLI/HTTP product owners plus the `UNRECEIPTED` C6A source-era cumulative-evidence candidate, normalized Darwin boot-session UUID, intent-only at-most-once spawn admission, exact readiness/raw exchange, bounded finalized run with separate five-domain scope, classifier-profile-bound terminal recovery, and sanitized deterministic evidence captures | retired/malformed/unstable/caller-authored boot profile or fallback, generic executable, copied/parsed target, dirty worktree, permit reissue, ambiguous-spawn reuse, runtime substitution, decoy-listener eligibility, metadata-shaped absence, forbidden dependency, contradiction/ineligibility collapse, private-evidence leak, hidden execution head, capture substitution, self-receipt, critic laundering, or portability overclaim |
...
## U9 export and local package addendum

U9 adds a local, deterministic, script-free report over one server-issued immutable seed snapshot. The default projection omits captured bodies, environment values, private paths, process streams, credentials, candidate identities, and producer/model provenance. It exposes only the selected outcome fields and applies one versioned known-pattern redaction pass to their displayed values. Stimulus previews retain names, counts, modes, and digests while omitting bodies and environment values. This is fixture-profile minimization, not arbitrary-secret detection: **`CONFIDENTIALITY NOT ESTABLISHED`**. Raw output requires two explicit danger flags and may contain every retained canonical artifact.

The report uses structural HTML escaping and a `default-src 'none'` / no-script CSP, embeds no remote resource, bounds visible fields and total output, and publishes mode-`0600` bytes by an exclusive same-directory hard-link commit. It refuses an existing destination and a symlink final parent. Ancestor aliases provided by the operating system are canonicalized before publication. These controls do not establish hostile same-user replacement resistance, secure deletion, safe sharing, or complete browser threat coverage.

The local package embeds and verifies the exact three-file production web roster before binding the studio. Its smoke test runs the copied binary in a fresh private root, checks help/version/export, literal loopback binding, fragment-token bearer flow, static assets, private output, and terminal shutdown. The binary is unsigned and locally built; there is no release publication, signing, SBOM, provenance attestation, or trademark/package-name clearance. Node remains the declared fixture/generated-contract runtime for the exercised studies rather than an embedded Countershape runtime.
