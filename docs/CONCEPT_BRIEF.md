...
**Active U7D source-candidate snapshot:** exact sealed U7C commit `cbc908d240a433b703827f6c5ae397be63588d1a`, tree `2dd1fb793d0cf118d23581598b497d3cce00b5c9`, is the direct parent. U7D is the exact 15-path `SOURCE_FULL` cross-domain closure candidate with subject `test: close U7 reference study evidence`, product authority `U7_REFERENCE_APPLICATION`, behavior `CANDIDATE_UNRECEIPTED`, 13 `UNRECEIPTED` claims, and receipt `ABSENT`. Its strongest permitted milestone is `LOCAL_TWO_DOMAIN_PROCESS_ARTIFACT_GREEN_CLI_OFFICIAL_EXECUTION_HTTP_DIRECT_PROCESS_ONLY_FULL_STUDY_RESOURCE_UNRECEIPTED`; HTTP finalized-run/classification, two-domain target/run/classification reproduction, generalized CLI execution, and full-study resources remain unvalidated. Only separately sealed U7R may reconcile the source receipt.
