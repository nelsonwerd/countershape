...
| `ContractExecutionTarget` | immutable nonhead pre-spawn authority constructed from the reopened bundle/residue, a live Git-issued explicit pinned and verified materialization, a fresh durable conformance attempt, opaque `DARWIN_KERN_BOOTSESSIONUUID_V1` authority, and an admitted/revalidated Node runtime | official only through its typed store-private witness and exact link; parsed bytes, copied OIDs/digests/runtime fields, and caller-authored boot values are inert; no process-result or study-head claim |
