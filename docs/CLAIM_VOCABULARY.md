...
| `ContractExecutionTarget` | immutable nonhead pre-spawn authority constructed from the reopened bundle/residue, a live Git-issued explicit pinned and verified materialization, a fresh durable conformance attempt, opaque `DARWIN_KERN_BOOTSESSIONUUID_V1` authority, and an admitted/revalidated Node runtime | `Valid` is structural; the matching profile owner requires immediate physical reopen after owner acquisition immediately before permit consumption/`Start`; sealed C4 uses one detached bounded closure context, while active C5 hands a bounded revalidation phase directly to a separately bounded closure phase and checks revalidation expiry before consumption; the fixed probe is cooperating-executable profile evidence, not Node vendor authenticity; parsed bytes, copied fields, and caller-authored boot values are inert; no process-result or study-head claim |
...
Historical sealed-C4 wording remains literal: C4 requires immediate physical reopen at its owned admission boundary. The active C5 owner has its own corresponding reopen chronology; this sentence does not make C4 current.
...
The active C5 development matrix observes HTTP 404 as `CONFORMS`, differing HTTP 403 as `CONTRADICTS`, a custom-401 predicate against observed 403 as `CONTRADICTS`, an allow-many set containing 403/404 as `CONFORMS`, and an allow-many set excluding observed 200 as `CONTRADICTS`. These are exact fixture/profile outcomes, not arbitrary HTTP compatibility, and remain `UNRECEIPTED` until the complete C5 79-claim ledger commits, seals, and verifies strictly.
