...
| `ContractExecutionTarget` | immutable nonhead pre-spawn authority constructed from the reopened bundle/residue, a live Git-issued explicit pinned and verified materialization, a fresh durable conformance attempt, opaque `DARWIN_KERN_BOOTSESSIONUUID_V1` authority, and an admitted/revalidated Node runtime | `Valid` is structural; C4 requires immediate physical reopen; the fixed probe is cooperating-executable profile evidence, not Node vendor authenticity; parsed bytes, copied fields, and caller-authored boot values are inert; no process-result or study-head claim |
