...
- **Status:** controlling language contract frozen at the C6A source-candidate epoch; product capability through C5 and verifier maintenance through C5V are sealed, the C6A receipt is absent, and only a sealed per-unit receipt map may state a didrun grade
...
| `ContractExecutionTarget` | immutable nonhead pre-spawn authority constructed from the reopened bundle/residue, a live Git-issued explicit pinned and verified materialization, a fresh durable conformance attempt, opaque `DARWIN_KERN_BOOTSESSIONUUID_V1` authority, and an admitted/revalidated Node runtime | `Valid` is structural; the matching profile owner requires immediate physical reopen after owner acquisition immediately before permit consumption/`Start`; sealed C4 uses one detached bounded closure context, while sealed C5 hands a bounded revalidation phase directly to a separately bounded closure phase and checks revalidation expiry before consumption; the fixed probe is cooperating-executable profile evidence, not Node vendor authenticity; parsed bytes, copied fields, and caller-authored boot values are inert; no process-result or study-head claim |
...
| `StartClaim` / `RunPermit` | durable target-keyed intent permanently consumes one target; only the matching admitted profile-specific execution owner that wins both authentic-target interlock and claim acquisition receives one process-local, nonreopenable, single-consumption permit | the current exact owners are the sealed CLI runner and sealed C5 HTTP runner; serialized cooperative at-most-once spawn admission only; no proof a child started, stopped, or is absent; fresh target alone cannot bypass an ambiguous held interlock |
...
| `child-reported HTTP service binding` | one exact accepted FD-3 portable readiness frame followed by EOF, with its exact reported endpoint consumed for one raw loopback connection attempt and child/service facts retained separately | no listener ownership, endpoint authenticity, proxy absence, network denial, or hostile-process attestation |
| `HTTP invocation evidence` | canonical seven-member candidate-written receipt, at most 64 KiB, bound to one fresh attempt plus the exact stimulus/request digest and invocation count one, then retired by identity-bound closure | cooperating-candidate evidence only; not provenance, listener authenticity, or proof against same-UID replacement |
| `C5 HTTP private-evidence envelope` | pre-owner admission of at most 15 bodies: three framed bodies plus twelve canonical summaries under a source/profile-derived envelope capped by the inherited 64 MiB store ceiling | no confidentiality, secure deletion, perpetual availability, or permission to exceed any body/count/byte ceiling; every C5 grade remains `UNRECEIPTED` before the 79-claim seal |
| `C5 cooperating-attempt lease` | after root-metadata and declared-topology validation, one nonblocking no-follow temporary-root descriptor lease spans fixture-roster inspection, preparation, execution, receipt retirement, terminal persistence, and return; a live holder yields typed admission refusal, while process death releases the kernel lease | no persistent lock artifact, hostile same-UID isolation, PID-liveness or takeover authority, exactly-once execution, survivor-absence proof, or conversion of unowned crash/foreign residue from fixture rejection |
| `C5 exact seed roster` | compatible-partial descriptor preflight, exclusive fixed staging beneath the attempt temporary root even for an empty roster, complete-inode no-clobber publication, exact-empty staging retirement, and final exact declared directory/file roster | a point-in-time cooperating-code check; no hostile same-UID isolation, secure deletion, or guarantee against mutation after the final observation |
| `C5 process-pipe close diagnostics` | after `Start` is attempted, three fixed, deduplicated phase-level observations for close errors across the six parent-owned process-pipe endpoints, retained on the teardown-error axis | no proof a kernel descriptor remained open, no close retry, no primary-result replacement, no orphan-risk inference, and no claim about pre-Start cleanup, TCP, or every non-pipe close |
| `C5 classification-only recovery` | fresh live target reopen plus standalone-HTTP profile validation before terminal-store lookup, followed only by release and immutable classification convergence | bounded owned runtime probes may occur during reopen; no source preparation, owner acquisition, RunPermit, subject start, exchange, or respawn authority |
| `C6A evidence summary` | canonical tracked projection of three exact sealed-C5V cumulative timings, six named didrun bug statuses, one permanent negative, five fixed nonclaims, and an explicit private-evidence availability declaration | implementation-owned local evidence metadata only; no new product behavior, security verdict, human judgment, market signal, or C6A self-receipt |
| `C6A deterministic expert capture` | canonical schema-closed evidence derived from the exact C5V parent, exact present/absent source-byte authority, pure fixed profile descriptors, measured front-door executables, and exact selected CLI/HTTP/generated-Node profile transcripts | proves only the named local implementation facts and exact renderer input; no complete host-toolchain/SDK provenance, arbitrary-repository, vendor-authenticity, cross-platform, containment, or comprehension claim |
| `C6A width-constrained documentation capture` | ASCII-only deterministic projection of the canonical evidence and summary at one exact 60, 80, or 120 column ceiling | developer diagnostic, not product CLI ABI, terminal-cell portability, accessibility certification, or proof that a human understood it |
| `C6A critic disposition` | recorded response to a blind different-model review over sanitized tracked captures and a bounded rubric | critic identity, judgment, severity, and advice remain `UNRECEIPTED`, advisory, and semantically inert; deterministic capture receipts cannot promote qualitative judgment into correctness, taste, comprehension, or security |
| `HTTP scope-canary observation` | exact retained target inventory plus named import- and service-socket connection counts and exact omission of the two enrolled parent sentinel keys, closed in five ordered domains | no host-wide absence, arbitrary service discovery, listener ownership, egress denial, same-user containment, or claim about unmeasured environment state |
...
Historical sealed-C4 wording remains literal: C4 requires immediate physical reopen at its owned admission boundary. The sealed C5 owner has its own corresponding reopen chronology; this sentence does not make C4 current.

The sealed C4 physical fixture exercises both eligible CLI result branches without relabeling history: the enrolled reference target remains `CONTRADICTS`; a second real ruling compiled over the same exact two-file reference tree yields `CONFORMS` with the profile-ordered values `cli.stdout.json.mode=argv` and `cli.stdout.json.source=argv`.

The sealed C5 fixture matrix observes HTTP 404 as `CONFORMS`, differing HTTP 403 as `CONTRADICTS`, a custom-401 predicate against observed 403 as `CONTRADICTS`, an allow-many set containing 403/404 as `CONFORMS`, and an allow-many set excluding observed 200 as `CONTRADICTS`. These finite outcomes were receipted by exact sealed C5 under its 79/79 `TREE-EXACT` ledger and strict exit `0`; they establish no arbitrary HTTP compatibility and transfer no grade to C6A.
...
| U6c / P07B | recoverable deterministic residue plus only the separately receipted C target/run/classification layers and, after its own receipt descendants, exact C6 implementation evidence: typed publication, private boot-session interlock, serialized cooperative admission, exact runtime, closed process evidence, five-domain scope, and deterministic named-surface captures | current correctness, exactly-once execution, same-boot child absence after ambiguity, host-wide absence, listener ownership, broad portability, containment, confidentiality, authenticated-human authorship, comprehension, adoption, maintainership, or security review |
