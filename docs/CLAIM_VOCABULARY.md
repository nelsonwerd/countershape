...
| `ContractExecutionTarget` | immutable nonhead pre-spawn authority constructed from the reopened bundle/residue, a live Git-issued explicit pinned and verified materialization, a fresh durable conformance attempt, and an admitted/revalidated Node runtime | official only through its typed store-private witness and exact link; parsed bytes and copied OIDs/digests are inert; no process-result or study-head claim |
| `ExecutionInterlock` | store-private operational CAS serializes subject admission for one measured boot session and may block work after ambiguity | no semantic/result/discovery authority, liveness fact, process resume, or same-boot ambiguous reset; reset requires explicit operator action plus a measured boot-session identity demonstrably distinct from the held identity |
...
| `classifier profile link key` | canonical digest derived from the closed schema/kind/literal-profile declaration for a future exact C2 relationship | not serialized in `ContractExecution`, not caller-supplied classification authority, and no implementation-byte commitment |
